...
For a SPARQL join and empty-result lag demonstration, run
`cargo run --locked -p oxigraph --features text-index --example text_service`
([source](lib/oxigraph/examples/text_service.rs),
[query contract](docs/adr/0024-rebuildable-derived-indexes.md#g33-local-sparql-text-service-v1-2026-09-08)).
...
Anonymous serving defaults to loopback. Anonymous non-loopback binds require
`--unsafe-allow-remote-anonymous` consent before opening a store. See the
...
with socket context and pre-body admission. Both serve modes now provide
[`--access-policy`](cli/README.md#trusted-proxy-access-policy-fork): bounded
trusted-proxy identity, deny-by-default endpoint/whole-operation and direct
Graph Store authorization, separate operator access, atomic policy reload,
...
pre-commit rollback checks. Finite RDF and RDFS materialization now share cancellation
and the absolute request deadline across snapshot copying, FROM construction,
inference and query reads. RDFS also checks internal copies, sparse scans,
consistency/output assembly and memory estimation. Deadline-bound OWL and legacy
nontransactional bulk loading remain explicitly unsupported pending internal checkpoints. Resource accounting,
