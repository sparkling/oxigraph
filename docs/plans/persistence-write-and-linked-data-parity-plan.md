...
| G22 | Workload admission and operator resources | Opt-in global/class queues, reserved operator capacity, deadlines, request/result bytes, inner-join/sort/distinct/group/aggregate-DISTINCT rows, bounded admission telemetry and atomic file-backed reload implemented; old snapshots drain and startup transport ceilings cannot grow; principal fairness and broader resources remain open | Fuseki metrics/tasks and server controls | Query circuit breakers, timeouts, slow-query logging, and resource-aware operators | ADR-0027; native slices delivered, full workload/resource acceptance remains open |
