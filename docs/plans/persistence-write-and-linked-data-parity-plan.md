...
outside feed coverage. G2.3c retention/leases and bounded governance health are
implemented natively under ADR-0020. No new harness or dependency version was
added; SHA-256 reuses the existing workspace dependency.
See [ADR-0020's retention contract](../adr/0020-transactional-metadata-receipts-and-change-delivery.md#g23c-retention-and-governance-health-2026-09-07)
and the [runnable retention example](../../lib/oxigraph/examples/outbox_retention.rs).
...
open. The harness
gate was not G2.2 product progress. ADR-0020 was Proposed at that historical
checkpoint; its later native completion is recorded in the current outcome above.
...
| G03 | Transaction lifecycle and uncertain commit | G1.4a/b provides keyed outcome recovery and conservative storage-call failures; G2.3a adds atomic semantic receipts. Memory remains process-local; prepare, savepoints, active-state inspection, and phase-specific crash/power-loss proof remain absent | Explicit transaction lifecycle                                                                     | `begin`, `isActive`, `prepare`, `commit`, `rollback`, unknown state                                                                                                               | G2.3b-c native outbox/cursors and retention implemented; savepoints remain later scope |
...
| G05 | Prefix/namespace metadata                  | G2.1 transactional namespace registry is implemented across memory, RocksDB, and the rewritten-plane evaluator; parser prefixes remain transient unless explicitly persisted | Prefix mappings and a Fuseki prefix service                                                        | Transactional namespace operations                                                                                                                                                | Implemented G2.1; preserve explicit parser/SPARQL/dump opt-in |
| G06 | Durable change delivery                    | G2.2 acknowledged effects; G2.3a atomic receipts, G2.3b ordered bounded outbox and G2.3c retention/leases are implemented. Legacy writes remain outside feed coverage | RDF Patch and patch-log ecosystem                                                                  | Connection/store listeners; notifications                                                                                                                                         | G2.3b-c implemented; RDF Patch adapter optional |
| G07 | Commit receipt/idempotency                 | G2.3a atomic native receipts survive RocksDB reopen/backup; G2.3b-c add outbox/cursors, retention and permanent expired-key truth. Legacy keyed commits remain explicit | Transaction/log internals, not an Oxigraph-compatible receipt                                      | Explicit unknown-transaction-state error                                                                                                                                          | G2.3a-c implemented; never silently replay an indeterminate outcome |
| G08 | SHACL on write                             | G2.4a-b opt-in native staged-view gate, bounded policy receipts, and failure closure                                                                                                                                                                                                                                                                                                                                | SHACL Core/SPARQL and Fuseki validation endpoint                                                   | ShaclSail validates during commit                                                                                                                                                 | P1 pre-commit participant over staged view                                                                                  |
...
| G21 | Service identity and authorization         | Native ADR-0026 trusted-proxy identity and endpoint/whole-operation/direct Graph Store authorization now implemented; frozen promotion remains open; egress policy remains distinct                                                                                                                                                                                                                                                          | Fuseki authentication/authorization hooks and dataset access controls                              | Server security plus repository/application authorization integration                                                                                                             | ADR-0026; fail closed, keep reverse-proxy versus embedded authority explicit, and do not claim SPARQL row/graph filtering   |
| G22 | Workload admission and operator resources | Opt-in global/class/principal caps, bounded class priorities, reserved operator capacity, deadlines, request/result bytes, inner-join/OPTIONAL/MINUS/sort/distinct/group/aggregate-DISTINCT rows and path-buffer entries, fixed admission/resource observations and atomic file-backed reload implemented; old snapshots drain and startup transport ceilings cannot grow; weighted service shares and broader resources remain open | Fuseki metrics/tasks and server controls | Query circuit breakers, timeouts, slow-query logging, and resource-aware operators | ADR-0027; native slices delivered, full workload/resource acceptance remains open |
...
| P1.1-P1.2 namespaces, effects, receipts, outbox | [ADR-0020](../adr/0020-transactional-metadata-receipts-and-change-delivery.md)                                                                                                                                                  | G2.1 namespaces, G2.2 native staged/request/keyed capture, and G2.3a native atomic receipts implemented; G2.3b ordered outbox and G2.3c retention/leases/health are implemented natively under ADR-0020                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| P1.3 transaction-time SHACL                     | [ADR-0021](../adr/0021-transaction-time-shacl-validation.md)                                                                                                                                                                    | Implemented; G2.4a-b native gate, policy receipt, and failure closure                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| P1.4a-P1.4c readiness, backup, restore          | [ADR-0022](../adr/0022-operational-readiness-backup-and-recovery.md)                                                                                                                                                            | Implemented (native Rust/local CLI)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
and SPARQL joins are implemented. G3.4 native spatial queries/catch-up/SPARQL joins
are implemented; separate performance gates remain.
...
Current delivery: G2.4a provides the opt-in Rust `start_shacl_transaction` gate
over complete independently selected graphs, under the governed writer permit.
Mutable shapes, resulting graph topology, cancellation/deadline, cumulative
snapshot/result limits, both commit traits, and atomic reject/reopen behavior
have native tests. [ADR-0021](../adr/0021-transaction-time-shacl-validation.md)
is Implemented: G2.4b adds the bounded executable policy receipt, feature-independent
lookup, atomic expiry, and injected validation/rollback/ambiguous-commit closure.
Existing unguarded write APIs remain unguarded; no HTTP or binding
policy endpoint is claimed.
...
not mislabeled as failed. Denial-attempt counters preserve swallowed and nested
retrieval failures; returned SHACL commit-gate counters preserve validation
disposition independently of native commit outcome. Their bounded export and
native/CLI tests close G2.5 task `task-1787851231441-1gdfzd` and this P1.4a
observation contract. Automatic admission remains G4.2; the following P1.4b–c
slices now implement backup and restore.
...
Automated fresh-directory restore and contributor reconciliation are the
separately implemented P1.4c operation below, not part of package verification.
...
Implemented natively as G2.7 task `task-1787851233022-antw51`:
`Store::restore_backup` and CLI `restore` copy a verified package into a fresh
directory, validate storage and the complete retained outbox, reconcile exact
checkpoint/content/contributor identities, close read-only handles, and publish
a completion record last. Tests then open writable, query, roll back, commit and
restart independently of the removed original store. Invalid files, internal
indexes, outbox records, primary metadata, providers, and interrupted phases fail
closed. Optional artifact-bound local baselines report checkpoint-age intervals
...
G3.4 native spatial queries/catch-up/SPARQL joins are implemented;
separate provider performance gates remain.
...
The [G3.1 native provider](../adr/0023-statistics-and-bounded-join-planning.md#g31-native-physical-statistics-provider-2026-09-08)
now implements exact physical graph/predicate counts and empty topology, bounded
...
Native input slice implemented in task `task-1787851230690-xr6ls9`:
`Store::derived_snapshot` supplies a stable topology/RDF/namespace view plus
bounded complete-commit delta replay from the same snapshot. Physical checkpoint
...
The [opt-in spatial SPARQL binding](../adr/0024-rebuildable-derived-indexes.md#g34-local-sparql-spatial-service-v1-2026-09-08)
now joins distinct matching literals to ordinary RDF patterns on the same retained
snapshot, with independent source/generation context, bounded caching, fatal
cancellation and an explicit success marker for SILENT error handling. Native
tests cover all 24 relation names, graph/blank-node semantics and the committed/
rolled-back/reopened journey. Frozen performance/promotion receipts remain.
...
tests. Its native authenticated profile now exposes `--access-policy`, public
provider/authorizer traits, trusted-peer identity, whole-operation/direct Graph
...
general evaluator memory bound follows. Atomic file-backed workload reload now
retains one snapshot per attempt/queue/lease, uses coherent access-class
validation and cannot grow the startup listener transport envelopes. A distinct
loopback operator grant controls the bodyless reload route; old work drains and
new caps account prospectively for all occupancy. Native default/no-default
controller and HTTP tests support this source slice. Optional per-principal
active/queued data caps now share each trusted subject's allowance across
...
through reload; omission preserves FIFO. Independent OPTIONAL/MINUS right-hand
build rows now have a shared cumulative cap, including duplicate, nested and
repeated builds. Early refusal, failed streams, multi-operation owned rollback,
reload and restart are covered by native tests. Final-lease observations add
one fixed resource/phase pair (56 resource samples); existing inner-join
semantics and physical plans remain unchanged. Broader operator budgets, weighted
