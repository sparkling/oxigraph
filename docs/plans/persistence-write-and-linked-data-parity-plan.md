...
- Status: R1 delivered at source handoff `aa7128bb`; G2.2 capture/integration and
  ...
  implemented natively. G2.4a staged-view SHACL commit validation is the next product
...
expired receipt markers permanently block key replay. G2.4a staged-view SHACL
validation is next; G2.5–G2.7 operational readiness, backup receipts, and restore
...
| G06 | Durable change delivery                    | G2.2 returns captured effects after acknowledged commit; G2.3a stores receipts for recovery, but neither supplies a durable event feed | RDF Patch and patch-log ecosystem                                                                  | Connection/store listeners; notifications                                                                                                                                         | G2.3b ordered durable outbox next; RDF Patch adapter optional |
| G07 | Commit receipt/idempotency                 | G2.3a governed native commits atomically bind caller key, commit ID, sequence, and effect checksum; RocksDB receipt lookup survives reopen/backup without replay. Legacy keyed commits remain explicit; outbox/cursors and retention are outstanding | Transaction/log internals, not an Oxigraph-compatible receipt                                      | Explicit unknown-transaction-state error                                                                                                                                          | G2.3a implemented; G2.3b-c supplies feed and retention, never silent replay |
| G08 | SHACL on write                             | G2.4a opt-in native staged-view commit gate; G2.4b policy receipt outstanding                                                                                                                                                                                                                                                                                                                                | SHACL Core/SPARQL and Fuseki validation endpoint                                                   | ShaclSail validates during commit                                                                                                                                                 | P1 pre-commit participant over staged view                                                                                  |
...
| P1.3 transaction-time SHACL                     | [ADR-0021](../adr/0021-transaction-time-shacl-validation.md)                                                                                                                                                                    | Proposed; G2.4a native gate implemented; G2.4b outstanding                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
G2.4a native staged-view SHACL validation are implemented; G2.4b is next.
...
remains Proposed for G2.4b's durable executable policy receipt and injected-fault
closure. Existing unguarded write APIs remain unguarded; no HTTP or binding
