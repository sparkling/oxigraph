...
| G22 | Workload admission and operator resources | Opt-in global/class queues, reserved operator capacity, deadlines, request/result bytes, inner-join/sort/distinct rows and bounded admission telemetry implemented; principal fairness, reload and broader resources remain open | Fuseki metrics/tasks and server controls | Query circuit breakers, timeouts, slow-query logging, and resource-aware operators | ADR-0027; native slices delivered, full workload/resource acceptance remains open |
