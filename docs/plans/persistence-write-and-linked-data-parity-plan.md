...
precedence are implemented. Commits `d3af2e17` and `45121da9` also complete pure
benchmark-owner bundle verification and bounded canonical control-receipt
candidate replay over four builds, two controls, and 392 launches. It returns
`CANDIDATE_REPLAYED`, `finalDecisionEligible: false`, and `binding: null`; it
does not prove write-once creation, receipt-last order, fsync, mode/nlink,
inventory stability, live execution, or approval. G1.7 still owns human
...
qualification, and the separate human promotion decision. No physically sealed
...
| P0.5 compatibility/performance promotion        | [ADR-0017](../adr/0017-repository-evolution-and-evidence-promotion-harness.md), [ADR-0018](../adr/0018-transaction-guarantees-and-conflict-model.md), [ADR-0019](../adr/0019-unified-egress-cancellation-and-service-claims.md) | Registry, rejection evidence, exact G1.4b binding, Darwin-free legacy replay, v6 two-phase/statistics semantics, v7 identity, and pure owner/receipt-candidate replay are implemented. Human authorization, physical envelope/sealed replay and binding, live owners, final approval, benchmark, qualification, and human promotion remain open |
...
replay in `45121da9`. It is non-eligible and binding-null; the physical
receipt-last/write-once sealed-envelope replay that alone may derive the
prospective binding, current production owner emission, and a sealed
qualification receipt are absent. Ruflo map v14 records completed pure task
`task-1787892615000-rdwz7q`, pending physical task
`task-1787896401667-xookiy`, and corrected G1.7 progress at 55%. Legacy
v1/v3/v4/v5/v6 replay is Darwin-free and `LEGACY_REPLAY_ONLY`.
