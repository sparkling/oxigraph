...
contract v5 and its two-phase protocol are integrated and fail-closed: control
authorization and the final decision set remain proposed/unapproved, `run`
exits 4 before work and writes no G1.7 run, and canonical control-receipt replay
and production execution owners are absent. Exact G1.4b prerequisite binding
and Darwin-free v1/v3/v4 legacy replay are implemented. G1.7 still owns human
Phase A approval, sealed replay of permanently non-promoting negative/A/A
controls, the human Phase B final decision, current owner evidence, performance
qualification, and the separate human promotion decision. No control receipt,
...
| P0.5 compatibility/performance promotion        | [ADR-0017](../adr/0017-repository-evolution-and-evidence-promotion-harness.md), [ADR-0018](../adr/0018-transaction-guarantees-and-conflict-model.md), [ADR-0019](../adr/0019-unified-egress-cancellation-and-service-claims.md) | Registry, rejection evidence, pure owner replay, exact G1.4b prerequisite binding, Darwin-free legacy replay, and the v5 two-phase protocol are implemented. Human authorization, canonical control receipt/replay, live owners, final approval, benchmark, qualification, and human promotion remain open |
...
exact contract v5 `155c364b...`. Its control authorization `b9ee0f76...` and
final decision set `2fd16b9e...` are proposed/unapproved; `run` exits 4 at
`CONTROL_AUTH_PROPOSED` before identity, evidence, build, runtime-directory
creation, or sampling and writes no G1.7 run. Pure Agentic/native replay
validation is implemented over synthetic fixtures, but canonical control
receipt replay, current production owner emission, and a sealed qualification
receipt are absent. Legacy v1/v3/v4 replay is Darwin-free and
`LEGACY_REPLAY_ONLY`.
...
`d57eb7cb...`/`5a4f5721...`. Contract v5 implements the ordered two human
phases: first authorize only permanently non-promoting negative and
...
sample. No human approval or control execution has occurred. Current
clean-subject evidence, the benchmark, qualification, and the later human
