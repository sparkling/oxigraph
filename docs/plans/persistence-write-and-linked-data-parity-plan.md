...
contract v4 is integrated and fail-closed: all three decision artifacts remain
proposed/unapproved, `run` exits 4 before work, and the approved-fixture path
has no production execution owner. G1.7 still owns exact G1.4b receipt binding,
human authorization and sealed replay of permanently non-promoting negative/A/A
controls, the final bound decision set, current owner evidence, performance
qualification, and the separate human promotion decision. No benchmark result
exists.
...
| P0.5 compatibility/performance promotion        | [ADR-0017](../adr/0017-repository-evolution-and-evidence-promotion-harness.md), [ADR-0018](../adr/0018-transaction-guarantees-and-conflict-model.md), [ADR-0019](../adr/0019-unified-egress-cancellation-and-service-claims.md) | Registry, rejection evidence, pure owner-replay validation, and G1.4b lower-fault evidence are implemented. Current production owner emission/receipt, G1.4b outer binding, two-phase control authorization, final decisions, benchmark, qualification, and human promotion remain open |
...
exact contract v4 `dd97f4a2...` with decision set `9a76ace5...`. Reference,
performance, and noise are all `PROPOSED`/`UNAPPROVED`; `run` exits 4 before
identity, evidence, build, runtime-directory creation, or sampling. Even an
approved fixture stops at `G17_EXECUTION_OWNER_UNIMPLEMENTED`. Pure
Agentic/native replay validation is implemented over synthetic fixtures, but
current production owner emission and a sealed owner receipt are absent.
Legacy v1/v3 receipts remain structural replay-only, with eager current-Darwin
loading still an open legacy hardening task.

The accepted G1.4b receipt must be copied byte-for-byte, hash-bound, and
pure-replayed in the outer envelope. The circular direct-approval flow is
replaced by two human phases: first authorize only permanently non-promoting
negative and independently built A/A controls and seal/replay their raw owner
receipt; then bind that receipt and observed signature into the final atomic
reference/performance/noise decision before any subject/reference sample.
Current clean-subject evidence, the benchmark, qualification, and the later
human promotion decision remain open. There are no performance results.
