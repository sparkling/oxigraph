...
and every authority false. A successor isolation/request contract, attested
native `execveat` helper with a bounded status protocol, private co-located
physical build issuer, containment v2/native adapter,
control/sample/qualification execution owners, and
...
| G1.7 qualification and promotion                             | [ADR-0017](../adr/0017-repository-evolution-and-evidence-promotion-harness.md), [ADR-0018](../adr/0018-transaction-guarantees-and-conflict-model.md), [ADR-0019](../adr/0019-unified-egress-cancellation-and-service-claims.md) | Audited fail-closed v7 identity and v6 two-phase/statistics semantics, Darwin-free legacy replay, exact G1.4b binding, pure owner/receipt-candidate replay, physical envelope/current-state replay, dormant containment, exact source-workspace construction, and pure build/product evidence replay are implemented; the physical supervising build owner, native containment adapter, production control/sample/qualification owners, human approvals, live controls, qualification, and human promotion remain open; ADR-0018 remains Proposed |
...
- G1.4b is complete in product commit
  ...
  `task-1787871483413-ki34q2` row is in progress at 69%. Contract v7's
  ...
  `task-1787902127894-7n7vk3` is in progress at 86%, and containment task
  `task-1787902138074-0w648x` is in progress at 88%. The remaining sequence
  begins with the successor isolation/request contract, attested native
  `execveat` helper and status protocol, private co-located physical supervising
...
- The dedicated G1.7 qualification control is now exact contract v7
  ...
  Those additions
  confer no live G1.7
  build/control capability or authority. The full package suite contains 624
  tests: 621 pass, none fail, and three intentional host-gated cases are
  skipped. This is
...
| Task                                                  | Depends on                                                                            | Size | Exit gate                                                                                                                                                                                                                |
| ----------------------------------------------------- | ------------------------------------------------------------------------------------- | ---: | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| G2.1 Transactional namespace registry                 | G1.3-G1.4; evaluator freeze after `HARNESS-REGISTRY` and `HARNESS-REJECTION-EVIDENCE` |    M | Additive namespace capability passes memory/RocksDB/rewritten-plane commit, rollback, snapshot, ordering, legacy-open, corruption, and no-implicit-parser/serializer tests without changing the minimal write traits or dataset equality |
...
| G1.7 pure owner/receipt-candidate replay   | `task-1787892615000-rdwz7q`                                                                                                                         | Complete in `45121da9`; focused 45/45 and current full 621/0/3 pass; result is candidate-only, non-eligible, binding-null, and non-authoritative                                                  |
...
| G1.7 source workspace/build owner          | `task-1787902127894-7n7vk3`                                                                                                                         | In progress at 86%: `a457f46c` adds the exact execution plan/materializer; `c5687dac`/`3688ccda` add replay-only build claims; `da41d7e0` freezes the mount-namespace path policy; `c5050e9c` adds the raw supervisor; `13afa94d`/`3f8951e3` freeze and harden launch-ineligible request v1; `8b2c6366` adds replay-only process-evidence v3. The successor policy/request/helper/private issuer remains absent |
