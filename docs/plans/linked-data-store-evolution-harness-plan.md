...
- Observed fork source before this documentation slice:
  `0257587eefd3d6196c6b35176070adb4bb2cf780`
...
G1.7 contract v5 is integrated and hardened through `0257587e`. Its exact
control authorization and final decision set remain proposed/unapproved, so
`run` exits 4 before identity, evidence, build, or sampling and writes no G1.7
run. Legacy v1/v3/v4 replay is Darwin-free and the exact G1.4b prerequisite is
bound, but canonical control-receipt replay and the control/qualification
execution owners are absent. Human Phase A approval and sealed negative/A/A
...
| G1.7 qualification control                                | Outer receipt v1 and exact contract v5 `155c364b...`; control authorization `b9ee0f76...` and final decision set `2fd16b9e...` are proposed/unapproved. Pure fixtures cover strict current-v5 and Darwin-free legacy v1/v3/v4 replay; exact G1.4b projection/binding are `d57eb7cb...`/`5a4f5721...`                                                                                                                                                                                                          | `run` exits 4 before work and writes no G1.7 run. Canonical control-receipt replay, control/qualification owners, both human approvals, controls, samples, benchmark, qualification, and separate human promotion remain absent |
...
| G1.7 qualification and promotion                             | [ADR-0017](../adr/0017-repository-evolution-and-evidence-promotion-harness.md), [ADR-0018](../adr/0018-transaction-guarantees-and-conflict-model.md), [ADR-0019](../adr/0019-unified-egress-cancellation-and-service-claims.md) | Audited fail-closed v5 two-phase protocol, Darwin-free legacy replay, and exact G1.4b prerequisite binding implemented; owners, control receipt, human approvals, qualification, and human promotion remain open; ADR-0018 remains Proposed |
...
- G1.4b is complete in product commit
  ...
  `task-1787871483413-ki34q2` is in progress at 45%. Contract v5's two-phase
  protocol is implemented and audited, v1/v3/v4 legacy replay is Darwin-free,
  and the exact G1.4b receipt is bound at the current prerequisite boundary.
  The remaining sequence is genuine human Phase A control approval, sealed and
  replayed negative/A/A controls from the live control owner, human Phase B's
  final decision binding, regenerated current owner evidence, the isolated
  paired benchmark and owner replay, and a separate human promotion decision.
...
- The dedicated G1.7 qualification control is now exact contract v5
  `155c364b...`. The proposed control-authorization and final-decision raw
  SHA-256 values are `b9ee0f76...` and `2fd16b9e...`. A current `g1.7:run`
  ...
  Legacy v1/v3/v4 replay is Darwin-free. The exact accepted G1.4b prerequisite
  projection/binding is `d57eb7cb...`/`5a4f5721...`. The full package suite
  contains 391 tests: 389 pass, none fail, and two intentional host-gated cases
  are skipped. This is fail-closed protocol evidence, not human approval,
  control-receipt evidence, semantic qualification, benchmark evidence, a
  performance result, or promotion.
- Contract v5 implements the ordered lifecycle. Human Phase A may authorize
...
| G1.7 two-phase owner programme             | `task-1787875911749-oh5io9` / `task-1787875912935-c2bi69` / `task-1787875914136-aw7onh` / `task-1787875915280-otfrw9` | V5 protocol implementation/hardening complete; human approvals, sealed control owner/receipt/replay, final approval, and qualification owner remain    |
| G1.7 prerequisite / legacy / governance    | `task-1787875916428-3vb8m7` / `task-1787875917561-6s54e3` / `task-1787875918734-1sjl6c`                               | Exact G1.4b prerequisite binding and Darwin-free legacy dispatch complete; v9 governance synchronization in progress                                   |
