...
92%. Commits `78b2cf99` through `65fb0e7a` freeze schema-v1 compatibility and
...
remains 92%; the separate ADR-0035 native task is 75% in progress. This remains
unregistered and non-authoritative: filesystem-backed guardian/reaper
durability and recovery, delegated-cgroup evidence, race-free exec and
pidfd/waitid binding, the physical adapter and full path-executed runtime-
closure proof, application receipt v7/replay, evaluator reconstruction, and
...
| Pinned source checkouts                                   | G0.1 initialized and verified RDF Canon, JSON-LD API, JSON-LD Streaming, and historical N3 pin `b975fc59`; the independently reviewed dependency-maintenance slice now selects local-only N3 `8a9ea8e`, with its audit/build/supply-chain receipt separate from the old semantic result                                                                                                                                                                                                                       | Local verifiers must initialize the currently registered revisions rather than substitute parent HEAD. The N3 208/871/296 result remains historical evidence for `b975fc59` and is not current semantic evidence for `8a9ea8e`; the selected commit is not on origin, so separately authorized submodule publication must precede any parent push, and no qualification or readiness lift follows from the repin                                                 |
...
G4 product capability. It must be implemented before G2.2 admits a new module,
but its completion earns no G2.2 product progress by itself.
...
| `HARNESS-CREATE-EXACT` dormant schema-v2 pre-registration    | [ADR-0034](../adr/0034-first-class-exact-new-file-admission.md)                                                                                                                                                                 | Proposed, broader task 92%: existing v2 primitives and the authority-null checkpoints remain unregistered. ADR-0034 owns receipt-v7/replay, reconstruction, profile/command/CLI literals, and an early exact qualification gate before workspace preparation; v1-v6 remain frozen                                                               |
...
| `HARNESS-CREATE-EXACT` Exact new-file admission       | [ADR-0017 controls](../adr/0017-repository-evolution-and-evidence-promotion-harness.md); G2.1 product checkpoint       |    M | [ADR-0034 task-contract schema v2](../adr/0034-first-class-exact-new-file-admission.md) proves exact absent baselines, literal Git-object lookup, canonical `A` status, phase-specific protected counts, worker instructions, deterministic reconstruction, unchanged task-level schema-v1 and application-receipt v1-v6 bytes, application-receipt v7 for v2, finite failures, and the exact early qualification gate before workspace preparation; task `task-1787935934614-ibmjn1` passes focused and full harness gates |
...
| Exact new-file admission / G2.2 gate       | `task-1787935934614-ibmjn1`                                                                                                                         | In progress at 92%: `7191ddde` adds the unregistered exact launch/bootstrap successor, `ab668ddd` adds the pure guardian journal, and `040f3343` adds the test-local preflight execution copy. Focused tests pass 47/47, top-level non-G1.7 tests excluding committed identity pass 495/495, and committed identity passes 2/2 on current Node and Node 20. ADR-0034 owns dormant receipt-v7/schema-v2 pre-registration and the early gate; ADR-0039 owns qualification/activation/runtime closure only. Filesystem-backed guardian/recovery, race-free exec plus pidfd/waitid evidence, the complete gates, a commit-capable successor, and G2.2 stay open; no product or promotion authority                                                                                      |
...
| ADR-0036 guardian static closure           | `task-1788114064268-vaigzl` / `task-1788114067364-wayobg` / `task-1788114070512-idkfkr` / `task-1788114073570-iba1gr`                               | B1-B4 complete through integrated commits `d971bfa4`, `86320201`, `99649146`, and `05bbd8bb`: 124/330 semantic negatives are frozen; `protectedAliases`, `indirectCalls`, `reflectComputed`, `bindingMemberWrites`, `untrustedSinks`, `rawEscapes`, and `literalMisuse` are closed. The full frozen corpus is 69 foundation negatives + 124 semantic negatives + 11 positives = 204 controls, leaving 206 semantic negatives. Exact pins, targeted mutation kills, import purity, and relevant current-Node/Node-20 matrices were independently reviewed. The candidate source remains absent, ADR-0036 remains Proposed, readiness remains unavailable/native-adapter-unavailable, and the checkpoint grants no runtime, G1.7, qualification, promotion, or publication authority. |
...
`task-1787935934614-ibmjn1` was read back through Ruflo's native task API at 92%
and remains in progress. The separate ADR-0035 task
