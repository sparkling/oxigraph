...
G1.7 contract v6 is integrated and independently audited through `799307fb`.
Its exact control authorization and final decision set remain
proposed/unapproved, so `run` exits 4 before identity, evidence, build, or
sampling and writes no G1.7 run. Legacy v1/v3/v4/v5 replay is Darwin-free and
the exact G1.4b prerequisite is bound. V6 freezes canonical sample bytes,
paired negative/A/A estimators and seeds, the MAD boundary, and mechanical
verdict precedence, but canonical control-receipt replay and the
control/qualification execution owners are absent. Human Phase A approval and sealed negative/A/A
...
| G1.7 qualification control                                | Outer receipt v1 and exact contract v6 `22cec755...`; control authorization `285c86fd...` and final decision set `e2884b73...` are proposed/unapproved. Pure fixtures cover strict current-v6, exact v5 archives, Darwin-free legacy v1/v3/v4/v5 replay, authorization-bound sample bytes, paired control statistics, and exact G1.4b projection/binding `d57eb7cb...`/`5a4f5721...`                                                                                                                          | `run` exits 4 before work and writes no G1.7 run. Canonical control-receipt replay, control/qualification owners, both human approvals, controls, samples, benchmark, qualification, and separate human promotion remain absent |
...
| G1.7 qualification and promotion                             | [ADR-0017](../adr/0017-repository-evolution-and-evidence-promotion-harness.md), [ADR-0018](../adr/0018-transaction-guarantees-and-conflict-model.md), [ADR-0019](../adr/0019-unified-egress-cancellation-and-service-claims.md) | Audited fail-closed v6 two-phase/statistics protocol, Darwin-free legacy replay, and exact G1.4b prerequisite binding implemented; owners, control receipt, human approvals, qualification, and human promotion remain open; ADR-0018 remains Proposed |
...
- G1.4b is complete in product commit
  ...
  `task-1787871483413-ki34q2` is in progress at 45%. Contract v6's two-phase
  protocol and exact paired control statistics are implemented and audited,
  v1/v3/v4/v5 legacy replay is Darwin-free, and the exact G1.4b receipt is
  bound at the current prerequisite boundary. This harness-only slice does not
  increase product progress. The remaining sequence is pure benchmark-owner
  bundle and canonical control-receipt replay, workspace/build/containment and
  live control owners, genuine human Phase A control approval, sealed and
  replayed negative/A/A controls, human Phase B's final decision binding,
  regenerated current owner evidence, the isolated paired benchmark and owner
  replay, and a separate human promotion decision.
...
- The dedicated G1.7 qualification control is now exact contract v6
  `22cec755...`. The proposed control-authorization and final-decision raw
  SHA-256 values are `285c86fd...` and `e2884b73...`. A current `g1.7:run`
  ...
  Legacy v1/v3/v4/v5 replay is Darwin-free. V6 exact-binds canonical sample
  ...
  prerequisite projection/binding is `d57eb7cb...`/`5a4f5721...`. The full
  package suite contains 411 tests: 409 pass, none fail, and two intentional host-gated cases
...
- Contract v6 preserves the ordered lifecycle. Human Phase A may authorize
