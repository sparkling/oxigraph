...
surface; any alternative must begin as a separately authorized, non-default
semantic falsification experiment and then satisfy operations, migration, and
matched RDF workload gates. This decision adds no adapter, dependency,
benchmark result, migration, qualification, or production claim.
...
ADR-0037 delivery is frozen as an evaluator-first task graph and remains
unstarted. The local ADR-0036 C21 and programme-umbrella closure recorded by the
commit carrying this synchronization satisfies S0's dependency gate; S0
(`task-1788205371168-e6caq3`) is dependency-eligible but remains pending at zero
progress and unstarted. It freezes the ADR-only contract. S1
(`task-1788205386056-qz0anv`) and S3
(`task-1788205431734-trylt5`) add source-absent evaluators; replacement S2
(`task-1788399091085-u7a5lw`) may add only the pure statefs source. S4A
(`task-1788399095038-1zb8po`) freezes the private-filesystem integration RED
before S4B (`task-1788399098734-v8ve9g`) may add the three native syscall
artifacts. S5 (`task-1788399104822-pzsod1`) is read-only integration
verification. In parallel after S2, S6A (`task-1788399108388-65439n`) freezes
the manager-protocol RED before S6B (`task-1788399112386-qn8psc`) may add its
sole source. S7 (`task-1788399842562-kjszvk`) joins S5 and S6B and releases
only corrected ADR-0038 S0 (`task-1788399847418-arzwjy`). Every GREEN slice is
forbidden from changing its frozen evaluator; a defect requires a new bounded
evaluator-first repair task. This task-map correction changes no code, ADR
status, readiness, qualification, promotion, publication, or authority.
...
| Persistent-backend adoption gate                             | [ADR-0042](../adr/0042-retain-rocksdb-and-gate-replacement-backend-experiments.md)                                                                                                                                              | Proposed evidence-backed defer/reject: RocksDB remains the only production-intended persistent backend; no replacement adapter is selected, and any future candidate starts as a non-default semantic falsification experiment                                                                                                                                                                     |
...
| G1.7 private build issuer and physical owner chain           | [ADR-0041](../adr/0041-g17-private-co-located-build-issuer.md)                                                                                                                                                                  | Proposed; S0 `task-1788403444941-5l8jci` is complete. Local C21/umbrella closure makes S1 `task-1788403485637-t9wn40` eligible but pending at 0% and unstarted. Physical issuance still requires ADR-0037 through ADR-0040, successor activation, and explicit human G1.7 authority                                                                                                                |
...
| Replacement-backend decision               | `task-1788074788516-p5tvdm`                                                                                                                                                                                                                                                                               | [ADR-0042](../adr/0042-retain-rocksdb-and-gate-replacement-backend-experiments.md) records the evidence-backed TurboKV defer/reject boundary: retain RocksDB, select no replacement adapter, and require a separately authorized non-default semantic experiment before workload or adoption work                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| ADR-0036 C21 local documentation closure   | `task-1788204883871-l9tsh9`                                                                                                                                                                                                                                                                               | Locally closed by this synchronization against C17 `b037ed0a` / tree `64cb90bc` and approved C18-C20; exact task/umbrella read-back follows the commit. ADR-0036 stays Proposed and unavailable. External publication moved to `task-1788409495130-6ikk41`; neither Gist nor main was updated.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| ADR-0037 evaluator-first delivery DAG      | `task-1788205371168-e6caq3` / `task-1788205386056-qz0anv` / `task-1788399091085-u7a5lw` / `task-1788205431734-trylt5` / `task-1788399095038-1zb8po` / `task-1788399098734-v8ve9g` / `task-1788399104822-pzsod1` / `task-1788399108388-65439n` / `task-1788399112386-qn8psc` / `task-1788399842562-kjszvk` | Planned and unstarted; local C21/umbrella closure makes S0 eligible but pending at 0% and unstarted. RED/GREEN separation and every later dependency/authority gate remain unchanged. Receipt: `task-plans/adr-0037-evaluator-first-dag-v2-2026-09-03`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |

Beneath the preserved pre-C16 and pre-C17 planning rows, the three ADR-0036
rows record the current C17-C21 state and the following ADR-0037 row records
its eligible-but-unstarted delivery DAG. Earlier task prose used pre-C15 10/15/25
...
| ADR-0042 replacement-backend defer/reject decision                      | `task-1788074788516-p5tvdm`                                                                                                                         |
...
ADR-0041's documentation-only S0 is complete. Local ADR-0036 C21 and umbrella
closure satisfies S1's C21 dependency, so S1 `task-1788403485637-t9wn40` is
dependency-eligible but remains pending at zero progress and unstarted. S2 may
implement the pure requirements contract only after S1 freezes evaluator RED.
S3 cannot start until genuine ADR-0037 and ADR-0038 integrated
...
umbrella is `task-1788403413560-sedu3a`; its exact child DAG, original C21 hold,
distinct host-positive gate, four-review closure, and cancelled-v1 history are
stored and read back at
`task-plans/adr-0041-evaluator-first-dag-v2-2026-09-03`. The v1 record and all
superseded task rows remain immutable historical evidence. The C21 dependency is
now locally satisfied, but S1 remains pending at zero progress and unstarted.
