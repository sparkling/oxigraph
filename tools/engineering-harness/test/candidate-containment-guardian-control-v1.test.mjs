...
function evaluateC14Provenance(
  ...
  const authorityKeys = Object.freeze(
    Object.keys(REQUIREMENTS_ORACLE.authority),
  );
  ...
  for (const statement of program.body) {
    if (statement.type === "ImportDeclaration") {
      ...
      if (allowed === undefined) {
        throw new Error(
          "static gate: provenance PINNED_IMPORT_SOURCE_MISMATCH",
        );
      }
      const observed = [];
      for (const specifier of statement.specifiers) {
        if (
          ...
          throw new Error(
            "static gate: provenance PINNED_IMPORT_NAME_MISMATCH",
          );
        }
      ...
      if (observed.join("\u0000") !== allowed.join("\u0000")) {
        throw new Error("static gate: provenance PINNED_IMPORT_NAME_MISMATCH");
      }
      continue;
    }
    if (statement.type === "VariableDeclaration") {
  ...
  if (
    ...
    semanticSha256(rootStaticValue) !== EXPECTED_REQUIREMENTS_SHA256
  ) {
  ...
  for (const [name, { init }] of moduleDeclarations) {
    ...
    if (
      ...
      semanticSha256(candidate) === EXPECTED_REQUIREMENTS_SHA256
    ) {
      invalidRequirementRoots.add(name);
    }
  }
  ...
  const exactFailureFunction = (name) => {
    ...
    return (
      ...
      STATIC_POLICY_REQUIREMENTS_ORACLE.vocabularies.failureCodes.includes(
...
function mainCommittedProtectedProjectionBody(schemaPath, storeName) {
...
function mainFrozenSourceShapePolicyControls() {
  const shapeFailure =
    'function failProvenanceShape() { throw new Error("CONTROL_SHAPE"); }';
  const boundsFailure =
    'function failProvenanceBounds() { throw new Error("CONTROL_BOUNDS"); }';
  const brandFailures =
    'function failProvenanceBrand() { throw new Error("CONTROL_BINDING"); } function failProvenanceRecord() { throw new Error("CONTROL_SHAPE"); }';
  const statusDecode =
    'const decoded = decodeCanonicalJsonLine(statusFrameBytes, "status frame", guardianContract.limits.statusFrameMaximumBytes, failProvenanceShape);';
  const selectionNormalize = `${statusDecode} const selection = exactRecord(decoded.value, guardianContract.frameFields.recoverySelection, "recovery selection", failProvenanceShape);`;
  const launchProjection = `${statusDecode} const frame = exactRecord(decoded.value, guardianContract.frameFields.ADMIT, "admission frame", failProvenanceShape); const capsuleBytes = decodeCanonicalBase64(frame.launchCapsuleV3, "launch capsule", guardianContract.limits.admissionFrameMaximumBytes, failProvenanceShape); const projection = verifyCandidateContainmentLaunchCapsuleV3(capsuleBytes);`;
  return Object.freeze([
    Object.freeze({
      id: "requirements-field-list-normalizer-leaf",
      accepted: true,
      source: provenanceRequirementsSkeleton(
        `${statusDecode} const normalized = exactRecord(decoded.value, guardianContract.frameFields.STATUS, "status frame", failProvenanceShape); return normalized.schema === guardianContract.schemas.wireFrame;`,
        shapeFailure,
      ),
    }),
    Object.freeze({
      id: "requirements-field-list-whole-record",
      accepted: false,
      expectedRejection:
        "static gate: ESTree provenance REQUIREMENTS_PATH_ESCAPE",
      source: provenanceRequirementsSkeleton(
        `${statusDecode} const normalized = exactRecord(decoded.value, guardianContract.frameFields, "status frame", failProvenanceShape); return normalized.schema;`,
        shapeFailure,
      ),
    }),
    Object.freeze({
      id: "requirements-numeric-bound-leaf",
      accepted: true,
      source: provenanceRequirementsSkeleton(
        'return boundedInteger(1, "status count", 0, guardianContract.limits.maximumStatusFramesPerTransition, failProvenanceBounds);',
        boundsFailure,
      ),
    }),
    Object.freeze({
      id: "requirements-numeric-bound-whole-record",
      accepted: false,
      expectedRejection:
        "static gate: ESTree provenance REQUIREMENTS_PATH_ESCAPE",
      source: provenanceRequirementsSkeleton(
        'return boundedInteger(1, "status count", 0, guardianContract.limits, failProvenanceBounds);',
        boundsFailure,
      ),
    }),
    Object.freeze({
      id: "requirements-strict-equality-leaves",
      accepted: true,
      source: provenanceRequirementsSkeleton(
        "const schemasDiffer = guardianContract.schemas.startupProjection !== guardianContract.schemas.statusArtifact; const mapsDiffer = guardianContract.startupMaps.normalSha256 !== guardianContract.startupMaps.recoveryOnlySha256; const vocabulariesDiffer = guardianContract.vocabularies.controlLevel !== guardianContract.vocabularies.controlType; return schemasDiffer && mapsDiffer && vocabulariesDiffer;",
      ),
    }),
    Object.freeze({
      id: "requirements-strict-equality-whole-subtree",
      accepted: false,
      expectedRejection:
        "static gate: ESTree provenance REQUIREMENTS_PATH_ESCAPE",
      source: provenanceRequirementsSkeleton(
        "return guardianContract.schemas === guardianContract.schemas;",
      ),
    }),
    Object.freeze({
      id: "requirements-array-receiver-leaves",
      accepted: true,
      source: provenanceRequirementsSkeleton(
        'const modePresent = guardianContract.modes.includes("NORMAL"); const kindPresent = guardianContract.vocabularies.inputKinds.includes("ADMIT"); const firstStateField = guardianContract.frameFields.stateProjection.at(0); return modePresent && kindPresent && firstStateField === "schema";',
      ),
    }),
    Object.freeze({
      id: "requirements-array-receiver-whole-record",
      accepted: false,
      expectedRejection:
        "static gate: ESTree provenance REQUIREMENTS_PATH_ESCAPE",
      source: provenanceRequirementsSkeleton(
        'return guardianContract.vocabularies.includes("ADMIT");',
      ),
    }),
    Object.freeze({
      id: "requirements-ternary-selected-numeric-leaves",
      accepted: true,
      source: provenanceRequirementsSkeleton(
        'const normal = guardianContract.modes.includes("NORMAL"); const count = normal ? guardianContract.startupMaps.normalDescriptorCount : guardianContract.startupMaps.recoveryOnlyDescriptorCount; return boundedInteger(count, "descriptor count", 0, guardianContract.limits.admissionRightsCount, failProvenanceBounds);',
        boundsFailure,
      ),
    }),
    Object.freeze({
      id: "requirements-ternary-selected-whole-record",
      accepted: false,
      expectedRejection:
        "static gate: ESTree provenance REQUIREMENTS_PATH_ESCAPE",
      source: provenanceRequirementsSkeleton(
        'const normal = guardianContract.modes.includes("NORMAL"); const count = normal ? guardianContract.startupMaps.normalDescriptorCount : guardianContract.startupMaps; return boundedInteger(count, "descriptor count", 0, guardianContract.limits.admissionRightsCount, failProvenanceBounds);',
        boundsFailure,
      ),
    }),
    Object.freeze({
      id: "protected-output-startup-authority-physical-facts",
      accepted: true,
      source: provenanceRequirementsSkeleton(
        mainCommittedProtectedProjectionBody(
    ...
    Object.freeze({
      id: "protected-output-input-authority-physical-facts",
      accepted: true,
      source: provenanceRequirementsSkeleton(
        mainCommittedProtectedProjectionBody(
    ...
    Object.freeze({
      id: "protected-output-state-authority-physical-facts",
      accepted: true,
      source: provenanceRequirementsSkeleton(
        mainCommittedProtectedProjectionBody(
    ...
    Object.freeze({
      id: "protected-output-status-authority-physical-facts",
      accepted: true,
      source: provenanceRequirementsSkeleton(
        'const bytes = copyBoundedBuffer(statusFrameBytes, "status frame", { minimumBytes: 0, maximumBytes: guardianContract.limits.statusFrameMaximumBytes }, failProvenanceBounds); return frozenCopyOnReadBytes(bytes, [["schema", guardianContract.schemas.statusArtifact], ["authority", guardianContract.authority], ["physicalFacts", guardianContract.physicalFacts]]);',
        boundsFailure,
      ),
    }),
    Object.freeze({
      id: "protected-output-swapped-authority-physical-facts",
      accepted: false,
      expectedRejection:
    ...
    Object.freeze({
      id: "state-exact-unsigned-preimage-hash",
      accepted: true,
      source: provenanceRequirementsSkeleton(
        mainStateHashProjectionBody(),
        "",
        "initializeCandidateContainmentGuardianControlV1",
      ),
    }),
    Object.freeze({
      id: "state-inexact-unsigned-preimage-hash",
      accepted: false,
      expectedRejection:
    ...
    Object.freeze({
      id: "pinned-import-normalized-dynamic-membership",
      accepted: true,
      source: provenanceRequirementsSkeleton(
        `${selectionNormalize} const plan = CANDIDATE_CONTAINMENT_RECOVERY_PLAN_STATUSES_V1.includes(selection.planStatus); const actor = CANDIDATE_CONTAINMENT_RECOVERY_ACTOR_KINDS_V1.includes(selection.actorKind); const disposition = CANDIDATE_CONTAINMENT_RECOVERY_DISPOSITIONS_V1.includes(selection.disposition); const location = CANDIDATE_CONTAINMENT_RECOVERY_SOURCE_LOCATIONS_V1.includes(selection.sourceLocation); const quarantine = CANDIDATE_CONTAINMENT_RECOVERY_QUARANTINE_REASONS_V1.includes(selection.quarantineReason); return plan && actor && disposition && location && quarantine;`,
        shapeFailure,
      ),
    }),
    Object.freeze({
      id: "pinned-import-raw-dynamic-membership",
      accepted: false,
      expectedRejection: "static gate: ESTree provenance PINNED_IMPORT_ESCAPE",
      source: provenanceRequirementsSkeleton(
        `${selectionNormalize} return CANDIDATE_CONTAINMENT_RECOVERY_PLAN_STATUSES_V1.includes(decoded.value.planStatus);`,
        shapeFailure,
      ),
    }),
    Object.freeze({
      id: "pinned-recovery-digest-normalized-comparison",
      accepted: true,
      source: provenanceRequirementsSkeleton(
        `${selectionNormalize} return selection.recoveryRequirementsSha256 === CANDIDATE_CONTAINMENT_RECOVERY_REQUIREMENTS_SHA256_V1;`,
        shapeFailure,
      ),
    }),
    Object.freeze({
      id: "pinned-recovery-digest-direct-escape",
      accepted: false,
      expectedRejection: "static gate: ESTree provenance PINNED_IMPORT_ESCAPE",
    ...
    Object.freeze({
      id: "pinned-launch-verifier-projection-chain",
      accepted: true,
      source: provenanceRequirementsSkeleton(
    ...
    Object.freeze({
      id: "pinned-launch-digest-direct-escape",
      accepted: false,
      expectedRejection: "static gate: ESTree provenance PINNED_IMPORT_ESCAPE",
      source: sourceSkeleton(
        "",
    ...
    Object.freeze({
      id: "pinned-launch-verifier-result-direct-escape",
      accepted: false,
      expectedRejection: "static gate: ESTree provenance PINNED_IMPORT_ESCAPE",
      source: provenanceRequirementsSkeleton(
        `${launchProjection} return projection;`,
        shapeFailure,
      ),
    }),
    Object.freeze({
      id: "pinned-launch-verifier-wrong-member",
      accepted: false,
      expectedRejection:
        "static gate: ESTree provenance PINNED_IMPORT_MEMBER_NOT_ALLOWED",
      source: provenanceRequirementsSkeleton(
        `${launchProjection} return projection.environment;`,
        shapeFailure,
      ),
    }),
    Object.freeze({
      id: "pinned-launch-verifier-file-direct-escape",
      accepted: false,
      expectedRejection: "static gate: ESTree provenance PINNED_IMPORT_ESCAPE",
      source: provenanceRequirementsSkeleton(
        `${launchProjection} const file = projection.files.at(0); return file;`,
        shapeFailure,
      ),
    }),
    Object.freeze({
      id: "local-helper-normalize-first-nested-value",
      accepted: true,
      source: provenanceRequirementsSkeleton(
        `${selectionNormalize} return readPlanStatus(selection);`,
        `${shapeFailure} function readPlanStatus(value) { return value.planStatus; }`,
      ),
    }),
    Object.freeze({
      id: "local-helper-raw-nested-value",
      accepted: false,
      expectedRejection:
        "static gate: ESTree provenance LOCAL_CALLER_UNTRUSTED",
      source: provenanceRequirementsSkeleton(
        `${selectionNormalize} return readPlanStatus(decoded.value);`,
        `${shapeFailure} function readPlanStatus(value) { return value.planStatus; }`,
      ),
    }),
    Object.freeze({
      id: "local-helper-branded-exact-record",
      accepted: true,
      source: provenanceRequirementsSkeleton(
        'const present = startupMetadata.has(startupProjection); exactBoolean(present, true, "startup projection brand", failProvenanceBrand); const metadata = startupMetadata.get(startupProjection); const normalized = exactRecord(startupProjection, guardianContract.frameFields.startupProjection, "startup projection", failProvenanceRecord); return readBrandedSchema(normalized);',
        `${brandFailures} function readBrandedSchema(value) { return value.schema; }`,
      ),
    }),
    Object.freeze({
      id: "local-helper-copied-and-decoded-bytes",
      accepted: true,
      source: provenanceRequirementsSkeleton(
        'const copied = copyBoundedBuffer(statusFrameBytes, "status frame", { minimumBytes: 0, maximumBytes: guardianContract.limits.statusFrameMaximumBytes }, failProvenanceShape); const decoded = decodeCanonicalJsonLine(statusFrameBytes, "status frame", guardianContract.limits.statusFrameMaximumBytes, failProvenanceShape); return trustedByteLength(copied) + trustedByteLength(decoded.bytes);',
        `${shapeFailure} function trustedByteLength(value) { return value.length; }`,
      ),
    }),
    Object.freeze({
      id: "local-helper-protected-import-result",
      accepted: true,
      source: sourceSkeleton(
        "function importedStateLength(value) { return value.length; }",
        new Map([
          [
            "verifyCandidateContainmentGuardianStatusFrameV1",
            "const state = CANDIDATE_CONTAINMENT_RECOVERY_RECORD_STATES_V1.at(0); return importedStateLength(state);",
          ],
        ]),
      ),
    }),
    Object.freeze({
      id: "local-helper-generated-normalized-transitive-callers",
      accepted: true,
      source: provenanceRequirementsSkeleton(
    ...
    Object.freeze({
      id: "local-failure-callback-normalizer-argument",
      accepted: true,
      source: provenanceRequirementsSkeleton(
        'const decoded = decodeCanonicalJsonLine(statusFrameBytes, "status frame", guardianContract.limits.statusFrameMaximumBytes, failExactLocal); const normalized = exactRecord(decoded.value, guardianContract.frameFields.STATUS, "status frame", failExactLocal); return normalized.schema;',
        'function failExactLocal() { throw new Error("CONTROL_SHAPE"); }',
      ),
    }),
    Object.freeze({
      id: "local-failure-callback-alias",
      accepted: false,
      expectedRejection: "static gate: ESTree provenance LOCAL_CALL_INDIRECT",
      source: provenanceRequirementsSkeleton(
        'const callback = failExactLocal; const decoded = decodeCanonicalJsonLine(statusFrameBytes, "status frame", guardianContract.limits.statusFrameMaximumBytes, callback); return decoded.bytes.length;',
        'function failExactLocal() { throw new Error("CONTROL_SHAPE"); }',
      ),
    }),
  ]);
...
function mainProvenancePolicyControls() {
  ...
  return Object.freeze([
    ...
    ...mainFrozenSourceShapePolicyControls(),
  ]);
}
...
const MAIN_C14_CANDIDATE_ACTIVATION_SCHEMA =
  "oxigraph.test.candidate-containment-guardian-control-v1-c14-candidate-activation/v1";
const MAIN_C14_CANDIDATE_ACTIVATION_MODE = "candidate-activation";
const MAIN_C14_CANDIDATE_STATE_COMMIT_REJECTION =
  "static gate: ESTree provenance STATE_COMMIT_KEY_NOT_EXACT";
const MAIN_C14_CANDIDATE_STATE_COMMIT_METADATA_REJECTION =
  "static gate: ESTree provenance STATE_COMMIT_METADATA_NOT_EXACT";
const MAIN_C14_CANDIDATE_STATE_METADATA_REJECTION =
  "static gate: ESTree provenance STATE_METADATA_NOT_EXACT";
const MAIN_C14_CANDIDATE_STATE_METADATA_STATE_REJECTION =
  "static gate: ESTree provenance STATE_METADATA_STATE_NOT_EXACT";
const MAIN_C14_CANDIDATE_STATE_REJECTION =
  "static gate: ESTree provenance STATE_PREIMAGE_NOT_EXACT";
const MAIN_C14_CANDIDATE_STATE_SHA256_RECORD_REJECTION =
  "static gate: ESTree provenance STATE_SHA256_RECORD_NOT_ALLOWED";
...
const MAIN_C14_CANDIDATE_STATE_METADATA_FIELDS = Object.freeze([
  "mode",
  "phase",
  "stateSha256",
  "eventCount",
]);
const MAIN_C14_PRODUCTION_AUDIT_SCHEMA =
  "oxigraph.test.candidate-containment-guardian-control-v1-c14-production-audit/v1";
const MAIN_C14_PRODUCTION_POLICY_SCHEMA =
  "oxigraph.test.candidate-containment-guardian-control-v1-c14-production-policy/v1";

function auditMainCandidateSourceForProduction(
  source,
  { audit = auditCandidateSource } = {},
) {
  assert.equal(typeof source, "string");
  assert.equal(typeof audit, "function");
  const legacyAudit = audit(source);
  ...
  if (
    candidateAudit?.c14Provenance?.mode !== MAIN_C14_CANDIDATE_ACTIVATION_MODE
  ) {
    throw new Error(
      `${MAIN_C14_CANDIDATE_ACTIVATION_SCHEMA}: missing composed candidate provenance pass`,
    );
  }
  return Object.freeze({
    ...legacyAudit,
    ...
    productionAudit: Object.freeze({
      schema: MAIN_C14_PRODUCTION_AUDIT_SCHEMA,
...
function mainCandidateActivationStateBody({
  ...
  const exactMetadata = "const metadata = stateProjectionMetadata(result);";
...
function mainCandidateActivationMetadataHelper(mutation = null) {
  const entries = [
    '["mode", state.mode]',
    '["phase", state.phase]',
...
function mainCandidateActivationSkeleton({
  ...
  const stateProjectionMetadataHelper =
    mainCandidateActivationMetadataHelper(metadataMutation);
  const source = sourceSkeleton(
    `${stateProjectionMetadataHelper}\n${extra}`,
    new Map([
      [
        "initializeCandidateContainmentGuardianControlV1",
        mainCandidateActivationStateBody({
...
function mainCandidateActivationControls() {
  const realSourceLimitFailure =
    'function failBounds() { throw new Error("CONTROL_BOUNDS"); }';
  const realSourceLimitBody =
    'const startupCarrier = copyBoundedBuffer(startupReportBytes, "startup report bytes", { minimumBytes: 0, maximumBytes: guardianContract.limits.startupReportMaximumBytes }, failBounds); const result = deepFreeze(nullRecord([])); const metadata = deepFreeze(nullRecord([])); startupMetadata.set(result, metadata); return result;';
  return Object.freeze([
    Object.freeze({
      id: "candidate-untagged-baseline-state-commit",
      source: sourceSkeleton(),
      expectedLegacyAccepted: true,
      expectedCandidateAccepted: false,
      expectedCandidateRejection: MAIN_C14_CANDIDATE_STATE_REJECTION,
    }),
    Object.freeze({
      id: "candidate-exact-state-and-four-key-metadata",
    ...
    ...["initialize", "reduce"].flatMap((owner) =>
      ["omitted", "reordered", "substituted", "arbitrary"].map(
        (preimageMutation) =>
          Object.freeze({
            id: `candidate-${owner}-${preimageMutation}-state-preimage`,
            source: mainCandidateActivationSkeleton(
              owner === "initialize"
    ...
    Object.freeze({
      id: "candidate-missing-protected-reducer-state-commit",
      source: mainCandidateActivationSkeleton({
    ...
    Object.freeze({
      id: "candidate-missing-protected-initialize-state-commit",
      source: mainCandidateActivationSkeleton({
    ...
    Object.freeze({
      id: "candidate-reducer-wrong-state-commit-key",
      source: mainCandidateActivationSkeleton({
        reduceWrongCommitKey: true,
      }),
      expectedLegacyAccepted: true,
      expectedCandidateAccepted: false,
      expectedCandidateRejection: MAIN_C14_CANDIDATE_STATE_COMMIT_REJECTION,
    }),
    Object.freeze({
      id: "candidate-initialize-wrong-state-commit-key",
      source: mainCandidateActivationSkeleton({
    ...
    ...[
      ...
      Object.freeze({
        id: `candidate-${metadataMutation}-state-projection-metadata`,
        source: mainCandidateActivationSkeleton({ metadataMutation }),
        expectedLegacyAccepted: true,
        expectedCandidateAccepted: false,
        expectedCandidateRejection: MAIN_C14_CANDIDATE_STATE_METADATA_REJECTION,
      }),
    ),
    Object.freeze({
    ...
    Object.freeze({
      ...
      expectedCandidateRejection:
        MAIN_C14_CANDIDATE_STATE_METADATA_STATE_REJECTION,
    }),
    Object.freeze({
      id: "candidate-wrong-state-commit-metadata-value",
      source: mainCandidateActivationSkeleton({
    ...
    Object.freeze({
      ...
      expectedCandidateAccepted: false,
      expectedCandidateRejection:
        MAIN_C14_CANDIDATE_STATE_SHA256_RECORD_REJECTION,
    }),
    Object.freeze({
      id: "candidate-real-startup-limits-leaf",
      source: mainCandidateActivationSkeleton({
    ...
    Object.freeze({
      id: "candidate-requirements-limits-subtree-escape",
      source: mainCandidateActivationSkeleton({
...
function mainC14FragmentControlProjection() {
...
function mainC14ProductionPolicyProjection(fragmentProjection) {
  return Object.freeze({
    schema: MAIN_C14_PRODUCTION_POLICY_SCHEMA,
    candidateMode: MAIN_C14_CANDIDATE_ACTIVATION_MODE,
...
function expectedMainC14ProductionFragmentReceipt(control) {
  return Object.freeze({
    schema: MAIN_C14_PRODUCTION_POLICY_SCHEMA,
    mode: "fragment-control",
    candidateMode: MAIN_C14_CANDIDATE_ACTIVATION_MODE,
    sourceSha256: byteSha256(Buffer.from(control.source, "utf8")),
    accepted: control.accepted,
    rejection: control.expectedRejection ?? null,
  });
}

function observeMainC14ProductionPolicyControls(controls) {
  return controls.map((control) => {
    ...
    try {
      MAIN_C14_PRODUCTION_AUDIT_BINDING(source, {
        audit(observedSource, stageAudit, mode) {
          sameDecodedSource &&= observedSource === source;
          if (mode !== MAIN_C14_CANDIDATE_ACTIVATION_MODE) {
            order.push("legacy");
          ...
          assert.equal(stageAudit, null);
          try {
            return auditCandidateSource(
              observedSource,
              candidateStageAudit,
              mode,
            );
...
function assertMainC14ProductionPolicyCoupling(fragmentProjection) {
  const controls = mainProvenancePolicyControls();
  const policyProjection =
    mainC14ProductionPolicyProjection(fragmentProjection);
  assert.equal(
    semanticSha256(policyProjection),
    EXPECTED_MAIN_C14_PRODUCTION_POLICY_PROJECTION_SHA256,
  );
  const observations = observeMainC14ProductionPolicyControls(controls);
  ...
  assert.deepEqual(
    ...
    controls.map((control) => ({
      ...
      candidateCallCount: 1,
      sameDecodedSource: true,
      fragmentReceipt: expectedMainC14ProductionFragmentReceipt(control),
    })),
    `${MAIN_C14_PRODUCTION_POLICY_SCHEMA}: candidate mode did not run the frozen fragment policy`,
  );
  assert.deepEqual(
    observations
    ...
    `${MAIN_C14_PRODUCTION_POLICY_SCHEMA}: fragment rejection drifted in production candidate mode`,
  );
}

function mainCandidateActivationProjection(controls) {
  return controls.map(
    ({
      id,
      source,
...
function observeMainCandidateActivationControls(controls) {
  return controls.map(({ id, source }) => {
    ...
    let candidateRejection = null;
    try {
      candidateAudit = MAIN_C14_PRODUCTION_AUDIT_BINDING(source, {
        audit(observedSource, stageAudit, mode) {
          assert.equal(observedSource, source);
          const phase =
            mode === MAIN_C14_CANDIDATE_ACTIVATION_MODE
              ? "candidate"
              : "legacy";
          order.push(phase);
          const observedAudit = auditCandidateSource(
            observedSource,
            stageAudit,
            mode,
          );
...
function expectedMainCandidateActivationObservations(controls) {
  return controls.map(
    ({
      id,
      expectedLegacyAccepted,
...
function assertMainCandidateProductionAuditBindingContract() {
  assert.equal(
    MAIN_C14_PRODUCTION_AUDIT_BINDING,
    auditMainCandidateSourceForProduction,
  );
  const source = "export const c14ProductionAuditProbe = null;\n";
  const calls = [];
  const audit = MAIN_C14_PRODUCTION_AUDIT_BINDING(source, {
    audit(...args) {
      const [observedSource, stageAudit = null, mode = "candidate-complete"] =
        args;
      calls.push(
        Object.freeze({
      ...
      return mode === MAIN_C14_CANDIDATE_ACTIVATION_MODE
        ? Object.freeze({
...
function assertMainCandidateActivationContract() {
  assertMainCandidateProductionAuditBindingContract();
  const fragmentProjection = mainC14FragmentControlProjection();
  ...
  assert.equal(
    semanticSha256(fragmentProjection),
  ...
  assert.deepEqual(MAIN_C14_CANDIDATE_STATE_METADATA_FIELDS, [
    "mode",
    "phase",
    "stateSha256",
    "eventCount",
  ...
  assert.equal(
    ...
    EXPECTED_MAIN_C14_CANDIDATE_ACTIVATION_PROJECTION_SHA256,
  );
  assertMainC14ProductionPolicyCoupling(fragmentProjection);
  const actual = observeMainCandidateActivationControls(controls);
  const expected = expectedMainCandidateActivationObservations(controls);
  assert.deepEqual(
    actual.at(0),
    expected.at(0),
    `${MAIN_C14_CANDIDATE_ACTIVATION_SCHEMA}: missing composed candidate provenance pass`,
  ...
  assert.deepEqual(
    actual.slice(1),
    expected.slice(1),
    `${MAIN_C14_CANDIDATE_ACTIVATION_SCHEMA}: candidate provenance contract mismatch`,
  );
}
