...
function directAssertContextualGrammar(
  program,
  expectedNodeCount,
  sourceSha256,
) {
  const directC14AuditedSource =
  ...
  const directC14AuditedDigestImports = new Set([
    "CANDIDATE_CONTAINMENT_LAUNCH_REQUIREMENTS_SHA256_V3",
    "CANDIDATE_CONTAINMENT_RECOVERY_REQUIREMENTS_SHA256_V1",
  ]);
  const directC14AuditedVocabularyImports = new Set([
    "CANDIDATE_CONTAINMENT_RECOVERY_ACTOR_KINDS_V1",
    "CANDIDATE_CONTAINMENT_RECOVERY_DISPOSITIONS_V1",
    "CANDIDATE_CONTAINMENT_RECOVERY_PLAN_STATUSES_V1",
    "CANDIDATE_CONTAINMENT_RECOVERY_QUARANTINE_REASONS_V1",
  ...
  for (const statement of program.body) {
    ...
    if (
      ...
      requirementsDependencies.add(declaration.init.name);
    }
  }
  const fail = (reason) => {
    throw new Error(`direct static gate: contextual ${reason}`);
  };
  const directC14AuditedNormativeLiteralValues = new Set();
  const collectDirectC14AuditedNormativeLiteralValues = (value) => {
    if (typeof value === "string") {
      directC14AuditedNormativeLiteralValues.add(value);
    } else if (Array.isArray(value)) {
      for (const entry of value) {
        collectDirectC14AuditedNormativeLiteralValues(entry);
      }
  ...
  if (directC14AuditedSource) {
    ...
    collectDirectC14AuditedNormativeLiteralValues(requirements);
  }
  ...
  const makeValue = (
    ...
    Object.freeze({
      ...
      compileTime: Object.freeze({
        ...
        iterableShape: compileTime?.iterableShape === true,
        iterationElement: compileTime?.iterationElement ?? null,
        iterationSummaryLeaf: compileTime?.iterationSummaryLeaf === true,
  ...
  const joinAbstractValuesWithSummaryMask = (summaryMask, ...values) => {
    ...
    const iterationElementForValue = (value) => {
      ...
      if (value.compileTime.iterableShape) {
        return makeCallerDerivedUnknownValue(hasCallerDerivedProvenance(value));
      }
      return null;
    };
    const iterationElements = values
      .map(iterationElementForValue)
      .filter((value) => value !== null);
    const iterableShape = values.some(
      (value) =>
        value.compileTime.iterableShape ||
        value.exactStrings.length > 0 ||
  ...
  const assertStaticStrings = (strings, literalRole) => {
    ...
    const capability = strings.find(
      (candidate) =>
        !candidateProjectionFieldNames.has(candidate) &&
  ...
  const sanitizeRequirementsValue = (
    value,
    kind,
    preserveArrayElementStrings = false,
  ) => {
    const keepString = (candidate) =>
      preserveArrayElementStrings || !capabilityLookingString(candidate);
    const sanitizedArrayElements =
      value.arrayElements === null
        ? null
    ...
    const sanitizedObjectProperties =
      value.objectProperties === null
        ? null
        : value.objectProperties.map(([name, entry]) => [
            name,
            sanitizeRequirementsValue(entry, "requirements-subtree"),
          ]);
    const removedString =
      value.staticStrings.some((candidate) => !keepString(candidate)) ||
      value.exactStrings.some((candidate) => !keepString(candidate)) ||
      value.compileTime.stringConversions.some(
        (candidate) => !keepString(candidate),
      );
    return makeValue(
      sanitizedArrayElements !== null || sanitizedObjectProperties !== null
        ? kind
        : "requirements-leaf",
      value.staticStrings.filter(keepString),
      value.exactStrings.filter(keepString),
      value.staticIntegers,
      sanitizedArrayElements,
      sanitizedObjectProperties,
      {
        ...value.compileTime,
        known: removedString ? false : value.compileTime.known,
        unknownString: removedString || value.compileTime.unknownString,
        stringConversions:
          value.compileTime.stringConversions.filter(keepString),
        arrayElementStringConversions:
          value.compileTime.arrayElementStringConversions.filter(keepString),
        plusStrings: value.compileTime.plusStrings.filter(keepString),
      },
    );
  };
  const declare = (
  ...
  let visitStatement;
  const directC14ContextuallyReachedLocalFunctions = new Set();
  const visitLiteral = (node, literalRole) => {
    ...
    if (typeof node.value === "string") {
      if (
        ...
        directC14AuditedNormativeLiteralValues.has(node.value) &&
  ...
  const evaluateIdentifier = (
    ...
    if (
      directC14AuditedSource &&
      binding.kind === "import-value" &&
      directC14AuditedDigestImports.has(binding.name)
    ) {
      return makeCallerDerivedTypedValue("string", {
        callerDerived: false,
        unknownMemberValue: false,
      });
    }
    if (
      directC14AuditedSource &&
      binding.kind === "import-value" &&
      directC14AuditedVocabularyImports.has(binding.name)
    ) {
      const element = makeCallerDerivedTypedValue("string", {
        callerDerived: false,
        unknownMemberValue: false,
      });
      return makeValue("frozen", [], [], [], null, null, {
        callerDerived: false,
        sequenceShape: true,
  ...
  const evaluateMember = (node, scope, context, { asCallee = false } = {}) => {
    ...
    const c14AtTopology = directC14AtTopologyExact();
    ...
    if (exactPinnedRecoveryStateAt) {
      ...
      if (binding.kind !== "import-value") {
        fail("pinned recovery state receiver");
      }
      return {
        kind: "safe-method",
        memberName,
        receiver: directC14PinnedRecoveryStatesValue(),
      };
    }
    if (node.object.type === "Identifier") {
      ...
      if (
        ...
        context.functionName === "stateProjectionMetadata" &&
  ...
  const exactC14DescriptorsAtDomain = (
    callNode,
    member,
    argumentValues,
    scope,
    ...
    if (
      !directC14AuditedSource ||
      context.functionName !== "validateStartupReport" ||
      member.memberName !== "at" ||
      member.receiver.kind !== "frozen" ||
      member.receiver.arrayElements?.length !== 8 ||
      callNode.callee.type !== "MemberExpression" ||
      callNode.callee.object.type !== "Identifier" ||
      callNode.callee.object.name !== "descriptors" ||
      callNode.arguments.length !== 1 ||
      callNode.arguments[0].type !== "MemberExpression" ||
      callNode.arguments[0].computed ||
      callNode.arguments[0].optional ||
      callNode.arguments[0].object.type !== "Identifier" ||
      callNode.arguments[0].object.name !== "expected" ||
      callNode.arguments[0].property.type !== "Identifier" ||
      callNode.arguments[0].property.name !== "fd"
    ) {
      return null;
    }
    const descriptorsBinding = resolve(scope, "descriptors");
    const descriptorsInit = descriptorsBinding.declarator?.init;
    const descriptorElements = descriptorsInit?.arguments?.[0]?.elements;
    if (
      descriptorsBinding.kind !== "local" ||
      descriptorsInit?.type !== "CallExpression" ||
      descriptorsInit.optional ||
      descriptorsInit.callee.type !== "Identifier" ||
      descriptorsInit.callee.name !== "deepFreeze" ||
      descriptorsInit.arguments.length !== 1 ||
      descriptorsInit.arguments[0].type !== "ArrayExpression" ||
      descriptorElements.length !== 8 ||
      !descriptorElements.every(
        (element, index) =>
          element?.type === "MemberExpression" &&
          !element.computed &&
          !element.optional &&
          element.object.type === "Identifier" &&
          element.object.name === "report" &&
          element.property.type === "Identifier" &&
          element.property.name === `fd${index}`,
      )
    ) {
      return null;
    }
    const expectedBinding = resolve(scope, "expected");
    const expectedMapSource = expectedBinding.forOfSource;
    const expectedMapBinding = resolve(scope, "expectedMap");
    const expectedMapInit = expectedMapBinding.declarator?.init;
    if (
      expectedBinding.kind !== "local" ||
      expectedMapSource?.type !== "Identifier" ||
      expectedMapSource.name !== "expectedMap" ||
      expectedMapBinding.kind !== "local" ||
      expectedMapInit?.type !== "ConditionalExpression" ||
      expectedMapInit.consequent.type !== "Identifier" ||
      expectedMapInit.consequent.name !== "normalStartupMap" ||
      expectedMapInit.alternate.type !== "Identifier" ||
      expectedMapInit.alternate.name !== "recoveryStartupMap"
    ) {
      return null;
    }
    const domain = argumentValues[0]?.staticIntegers ?? [];
    if (
      domain.length < 2 ||
      domain.length > 8 ||
      domain.some((index) => index < 0 || index >= 8) ||
      member.receiver.arrayElements.some(
        (element) => element.kind !== "immutable" || element.tainted === true,
      )
    ) {
      return null;
    }
  ...
  const directC14ExactMember = (node, rootName, path) => {
    let cursor = node;
    for (let index = path.length - 1; index >= 0; index -= 1) {
      if (
        ...
        cursor.property.name !== path[index]
      ) {
        return false;
      }
      cursor = cursor.object;
    }
    return cursor?.type === "Identifier" && cursor.name === rootName;
  };
  const directC14ExactCall = (node, calleeName, argumentCount) =>
    node?.type === "CallExpression" &&
    !node.optional &&
    node.callee.type === "Identifier" &&
    node.callee.name === calleeName &&
    node.arguments.length === argumentCount;
  const directC14ExactParameterList = (name, parameters) => {
  ...
  const directC14Walk = (node, visit) => {
    if (node === null || typeof node?.type !== "string") return;
    visit(node);
    for (const child of Object.values(node)) {
      if (Array.isArray(child)) {
        for (const entry of child) directC14Walk(entry, visit);
      } else {
        directC14Walk(child, visit);
      }
    }
  };
  const directC14NamedCallSites = (calleeName) => {
    const sites = [];
    for (const [callerName, record] of functionRecords) {
      directC14Walk(record.node.body, (node) => {
        if (
          node.type === "CallExpression" &&
          node.callee.type === "Identifier" &&
          node.callee.name === calleeName
        ) {
          sites.push({ callerName, node });
        }
      });
    }
    return sites;
  };
  const directC14IdentifierArguments = (node, names) =>
    node.arguments.length === names.length &&
    node.arguments.every(
      (argument, index) =>
        argument.type === "Identifier" && argument.name === names[index],
    );
  const directC14ExactCallTopology = (calleeName, expectedSites) => {
    const actualSites = directC14NamedCallSites(calleeName);
    return (
      actualSites.length === expectedSites.length &&
      expectedSites.every(
        ({ callerName, arguments: argumentNames }) =>
          actualSites.filter(
            ({ callerName: actualCaller, node }) =>
              actualCaller === callerName &&
              directC14IdentifierArguments(node, argumentNames),
          ).length === 1,
      )
    );
  };
  const directC14NullRecordEntries = (node) => {
    if (
      !directC14ExactCall(node, "deepFreeze", 1) ||
      !directC14ExactCall(node.arguments[0], "nullRecord", 1) ||
      node.arguments[0].arguments[0].type !== "ArrayExpression"
    ) {
      return null;
    }
    const entries = new Map();
    for (const pair of node.arguments[0].arguments[0].elements) {
      if (
        pair?.type !== "ArrayExpression" ||
        pair.elements.length !== 2 ||
        pair.elements[0]?.type !== "Literal" ||
        typeof pair.elements[0].value !== "string" ||
        entries.has(pair.elements[0].value)
      ) {
        return null;
      }
      entries.set(pair.elements[0].value, pair.elements[1]);
    }
    return entries;
  };
  let directC14NumericTopology = null;
  const directC14NumericTopologyExact = () => {
    if (directC14NumericTopology !== null) return directC14NumericTopology;
    const initialRecord = functionRecords.get("initialStateFields");
    const initialReturn = initialRecord?.node.body.body[0];
    const initialEntries =
      initialRecord?.node.body.body.length === 1 &&
      initialReturn?.type === "ReturnStatement"
        ? directC14NullRecordEntries(initialReturn.argument)
        : null;
    const zeroSeedExact = (key) => {
      const value = initialEntries?.get(key);
      return value?.type === "Literal" && value.value === 0;
    };
    directC14NumericTopology =
      ...
      directC14ExactCallTopology("initialStateFields", [
        {
          callerName: "initializeCandidateContainmentGuardianControlV1",
          arguments: ["startup"],
        },
      ]) &&
      directC14ExactCallTopology("appendStatusFields", [
        {
          callerName: "initializeCandidateContainmentGuardianControlV1",
          arguments: ["initialFields", "firstStatus"],
        },
        {
          callerName: "reduceCandidateContainmentGuardianControlV1",
          arguments: ["afterInput", "firstStatus"],
        },
        {
          callerName: "reduceCandidateContainmentGuardianControlV1",
          arguments: ["afterFirstStatus", "secondStatus"],
        },
      ]) &&
      directC14ExactCallTopology("applyInputFields", [
        {
          callerName: "reduceCandidateContainmentGuardianControlV1",
          arguments: ["state", "input", "plan"],
        },
      ]);
    return directC14NumericTopology;
  };
  const directC14ExactBoundedIntegerEnvelope = (node, context) => {
    const call = context.directCallNode;
    return (
      context.directCallArgumentIndex === 0 &&
      directC14ExactCall(call, "boundedInteger", 5) &&
      call.arguments[0] === node &&
      call.arguments[1].type === "Literal" &&
      call.arguments[1].value === "aggregate wire bytes" &&
      call.arguments[2].type === "Literal" &&
      call.arguments[2].value === 0 &&
      directC14ExactMember(call.arguments[3], "guardianContract", [
        "limits",
        "maximumAggregateWireBytes",
      ]) &&
      call.arguments[4].type === "Identifier" &&
      call.arguments[4].name === "failBounds"
    );
  };
  const directC14ExactCurrentStateBinding = (scope) => {
    const binding = resolve(scope, "state");
    const init = binding.declarator?.init;
    return (
      binding.kind === "local" &&
      directC14ExactCall(init, "exactRecord", 4) &&
      init.arguments[0].type === "Identifier" &&
      init.arguments[0].name === "currentState" &&
      resolve(scope, "currentState").kind === "parameter" &&
      directC14ExactMember(init.arguments[1], "guardianContract", [
        "frameFields",
        "stateProjection",
      ]) &&
      init.arguments[2].type === "Literal" &&
      init.arguments[2].value === "current state" &&
      init.arguments[3].type === "Identifier" &&
      init.arguments[3].name === "failShape"
    );
  };
  const directC14ExactCarrierBinding = (scope, specification) => {
    const binding = resolve(scope, "frameCarrier");
    const init = binding.declarator?.init;
    const bounds = init?.arguments?.[2];
    return (
      binding.kind === "local" &&
      directC14ExactCall(init, "copyBoundedBuffer", 4) &&
      init.arguments[0].type === "Identifier" &&
      init.arguments[0].name === specification.parameter &&
      resolve(scope, specification.parameter).kind === "parameter" &&
      init.arguments[1].type === "Literal" &&
      init.arguments[1].value === specification.label &&
      bounds?.type === "ObjectExpression" &&
      bounds.properties.length === 2 &&
      bounds.properties[0].type === "Property" &&
      !bounds.properties[0].computed &&
      bounds.properties[0].key.type === "Identifier" &&
      bounds.properties[0].key.name === "minimumBytes" &&
      bounds.properties[0].value.type === "Literal" &&
      bounds.properties[0].value.value === 0 &&
      bounds.properties[1].type === "Property" &&
      !bounds.properties[1].computed &&
      bounds.properties[1].key.type === "Identifier" &&
      bounds.properties[1].key.name === "maximumBytes" &&
      directC14ExactMember(bounds.properties[1].value, "guardianContract", [
        "limits",
  ...
  const directC14ExactNumericPairPlacement = (
    ...
    const matches = [];
    directC14Walk(record.node.body, (candidate) => {
      if (
        candidate.type === "ArrayExpression" &&
        candidate.elements.length === 2 &&
        candidate.elements[0]?.type === "Literal" &&
        candidate.elements[0].value === key
      ) {
        matches.push(candidate.elements[1]);
      }
    });
    ...
    return (
      functionName === "applyInputFields" &&
      key === "nextWireSequence" &&
      value?.type === "ConditionalExpression" &&
      value.test.type === "Identifier" &&
      value.test.name === "wireInput" &&
      value.consequent === node &&
      directC14ExactMember(value.alternate, "state", ["nextWireSequence"])
    );
  };
  const directC14ExactNumericAddition = (node, scope, context) => {
    if (
      !directC14AuditedSource ||
      node.operator !== "+" ||
      !directC14NumericTopologyExact()
    ) {
      return false;
    }
    const wireSpecifications = new Map([
      [
        "createCandidateContainmentGuardianAdmissionInputV1",
        {
          parameter: "admissionFrameBytes",
          label: "admission frame bytes",
          maximum: "admissionFrameMaximumBytes",
        },
      ],
      [
        "createCandidateContainmentGuardianCancelInputV1",
        {
          parameter: "cancelFrameBytes",
          label: "cancel frame bytes",
          maximum: "cancelFrameMaximumBytes",
        },
      ],
      [
        "createCandidateContainmentGuardianRecoveryRequestInputV1",
        {
          parameter: "recoveryRequestFrameBytes",
          label: "recovery request frame bytes",
          maximum: "recoveryRequestMaximumBytes",
        },
      ],
    ]);
    const wireSpecification = wireSpecifications.get(context.functionName);
    if (wireSpecification !== undefined) {
      return (
        directC14ExactBoundedIntegerEnvelope(node, context) &&
        directC14ExactMember(node.left, "state", ["aggregateWireBytes"]) &&
        directC14ExactMember(node.right, "frameCarrier", ["length"]) &&
        directC14ExactCurrentStateBinding(scope) &&
        directC14ExactCarrierBinding(scope, wireSpecification)
      );
    }
    if (
      context.functionName === "appendStatusFields" &&
      directC14ExactBoundedIntegerEnvelope(node, context)
    ) {
      return (
        directC14ExactMember(node.left, "fields", ["aggregateWireBytes"]) &&
        directC14ExactMember(node.right, "status", ["byteLength"])
      );
    }
    if (
      context.functionName === "applyInputFields" &&
      directC14ExactBoundedIntegerEnvelope(node, context)
    ) {
      return (
        directC14ExactMember(node.left, "state", ["aggregateWireBytes"]) &&
        directC14ExactMember(node.right, "input", ["frameByteLength"])
      );
    }
    const increments = new Map([
      ["appendStatusFields", "fields"],
      ["applyInputFields", "state"],
    ]);
    const root = increments.get(context.functionName);
    if (
      root === undefined ||
  ...
  const directC14DirectDeclarator = (functionName, name) => {
    const record = functionRecords.get(functionName);
  ...
  const directC14ExactFrozenOrdinalArray = (name, length) => {
    const initializer = moduleScope.bindings.get(name)?.declarator?.init;
    const elements = initializer?.arguments?.[0]?.elements;
    return (
      directC14ExactCall(initializer, "deepFreeze", 1) &&
      ...
      elements.every(
        (entry, index) =>
          ...
          entry.elements[0].value === index,
      )
    );
  };
  const directC14ExactFrozenMemberArray = (
    functionName,
    bindingName,
    rootName,
    prefix,
    ...
    const initializer = directC14DirectDeclarator(
      functionName,
      bindingName,
    )?.init;
    const elements = initializer?.arguments?.[0]?.elements;
    return (
      ...
      elements.every((element, index) =>
        directC14ExactMember(element, rootName, [`${prefix}${index}`]),
      )
    );
  };
  const directC14MemberAtSites = () => {
    const sites = [];
    for (const [functionName, record] of functionRecords) {
      directC14Walk(record.node.body, (node) => {
        if (
          node.type === "CallExpression" &&
          !node.optional &&
          ...
          sites.push({ functionName, node });
        }
      });
    }
    return sites;
  };
  const directC14ExactAtCall = (
    site,
    functionName,
    receiverMatcher,
    argumentMatcher,
    ...
    site.node.arguments.length === 1 &&
    receiverMatcher(site.node.callee.object) &&
    argumentMatcher(site.node.arguments[0]);
  let directC14AtTopology = undefined;
  const directC14AtTopologyExact = () => {
    if (directC14AtTopology !== undefined) return directC14AtTopology;
    const reject = () => {
      directC14AtTopology = null;
      return null;
    };
    if (
      !directC14AuditedSource ||
      !directC14ExactFrozenOrdinalArray("admissionRightOrdinals", 14) ||
      !directC14ExactFrozenOrdinalArray("recoverySlotOrdinals", 19) ||
      !directC14ExactFrozenOrdinalArray("recoveryDigestOrdinals", 28) ||
      !directC14ExactFrozenMemberArray(
      ...
      !directC14ExactFrozenMemberArray(
        "validateRecoveryStateSlots",
        "values",
        "selection",
        "state",
        19,
      ) ||
      !directC14ExactParameterList("validateAdmissionRights", [
        "report",
        "capsule",
        "state",
      ]) ||
      !directC14ExactParameterList("validateAdmissionRight", [
        "rightValue",
        "capsule",
        "scopeSha256",
        "expectedIndex",
      ]) ||
      !directC14ExactParameterList("validateRecoveryStateSlots", [
        "selection",
      ]) ||
      !directC14ExactParameterList("recoveryStateDigests", []) ||
      !directC14ExactCallTopology("validateAdmissionRights", [
        {
          callerName: "createCandidateContainmentGuardianAdmissionInputV1",
          arguments: ["report", "capsule", "state"],
        },
      ]) ||
      ...
      !directC14ExactCallTopology("recoveryStateDigests", [
        {
          callerName: "validateRecoveryStateSlots",
          arguments: [],
        },
      ]) ||
    ...
    const indexBinding = directC14DirectDeclarator(
      "validateAdmissionRight",
      "index",
    )?.init;
    const capsuleBinding = directC14DirectDeclarator(
      "createCandidateContainmentGuardianAdmissionInputV1",
      "capsule",
    )?.init;
    const digestsBinding = directC14DirectDeclarator(
      "validateRecoveryStateSlots",
      "digests",
    )?.init;
    if (
      !directC14ExactCall(indexBinding, "boundedInteger", 5) ||
      !directC14ExactMember(indexBinding.arguments[0], "right", ["index"]) ||
      indexBinding.arguments[1].type !== "Literal" ||
      indexBinding.arguments[1].value !== "admission right index" ||
      indexBinding.arguments[2].type !== "Identifier" ||
      indexBinding.arguments[2].name !== "expectedIndex" ||
      indexBinding.arguments[3].type !== "Identifier" ||
      indexBinding.arguments[3].name !== "expectedIndex" ||
      indexBinding.arguments[4].type !== "Identifier" ||
      indexBinding.arguments[4].name !== "failRights" ||
      !directC14ExactCall(
        capsuleBinding,
        "verifyCandidateContainmentLaunchCapsuleV3",
        1,
      ) ||
      capsuleBinding.arguments[0].type !== "Identifier" ||
      capsuleBinding.arguments[0].name !== "capsuleBytes" ||
      !directC14ExactCall(digestsBinding, "recoveryStateDigests", 0)
    ) {
      return reject();
    }
    const sites = directC14MemberAtSites();
    if (sites.length !== 39) return reject();
    const oneSite = (predicate) => {
      const matches = sites.filter(predicate);
      return matches.length === 1 ? matches[0].node : null;
    };
    const identifier = (name) => (node) =>
      node.type === "Identifier" && node.name === name;
    const literal = (value) => (node) =>
      node.type === "Literal" && node.value === value;
    const member = (root, path) => (node) =>
      directC14ExactMember(node, root, path);
    const descriptors = oneSite((site) =>
      directC14ExactAtCall(
        site,
        "validateStartupReport",
    ...
    const admissionOrdinal = oneSite((site) =>
      directC14ExactAtCall(
        site,
        "validateAdmissionRights",
        identifier("entry"),
    ...
    const rights = oneSite((site) =>
      directC14ExactAtCall(
        site,
        "validateAdmissionRights",
        identifier("rights"),
        identifier("expectedIndex"),
      ),
    );
    const capsuleFile = oneSite((site) =>
      directC14ExactAtCall(
        site,
        "validateAdmissionRight",
        member("capsule", ["files"]),
        identifier("index"),
      ),
    );
    const recoveryOrdinal = oneSite((site) =>
      directC14ExactAtCall(
        site,
        "validateRecoveryStateSlots",
        identifier("entry"),
    ...
    const previousValues = oneSite((site) =>
      directC14ExactAtCall(
        site,
        "validateRecoveryStateSlots",
        identifier("values"),
    ...
    const previousOrdinal = oneSite((site) =>
      directC14ExactAtCall(
        site,
        "validateRecoveryStateSlots",
        identifier("previousEntry"),
        literal(0),
      ),
    );
    const previousDigests = oneSite((site) =>
      directC14ExactAtCall(
        site,
        "validateRecoveryStateSlots",
        identifier("digests"),
        identifier("previousIndex"),
      ),
    );
    const currentOrdinal = oneSite((site) =>
      directC14ExactAtCall(
        site,
        "validateRecoveryStateSlots",
    ...
    const currentDigests = oneSite((site) =>
      directC14ExactAtCall(
        site,
        "validateRecoveryStateSlots",
        identifier("digests"),
    ...
    if (ordinarySites.some((node) => node === null)) return reject();
    const importedStates = sites
      .filter(
        (site) =>
          site.functionName === "recoveryStateDigests" &&
          site.node.callee.object.type === "Identifier" &&
          site.node.callee.object.name ===
            "CANDIDATE_CONTAINMENT_RECOVERY_RECORD_STATES_V1" &&
          site.node.arguments.length === 1 &&
          site.node.arguments[0].type === "Literal",
      )
      .sort((left, right) => left.node.start - right.node.start);
    if (
      importedStates.length !== 28 ||
      importedStates.some(
        ({ node }, index) => node.arguments[0].value !== index,
      )
    ) {
      return reject();
    }
    ...
    if (
      functionRecords.get("recoveryStateDigests")?.node.body.body.length !==
        1 ||
      recoveryReturn?.type !== "ReturnStatement" ||
      !directC14ExactCall(recoveryReturn.argument, "deepFreeze", 1) ||
      recoveryReturn.argument.arguments[0].type !== "ArrayExpression" ||
      recoveryElements.length !== 28 ||
    ...
    const recoveryRecord = functionRecords.get("validateRecoveryStateSlots");
    const dominatingSlotGuards = [];
    directC14Walk(recoveryRecord.node.body, (node) => {
      if (
        node.type === "IfStatement" &&
        node.test.type === "BinaryExpression" &&
        node.test.operator === ">" &&
        node.test.left.type === "Identifier" &&
        node.test.left.name === "slot" &&
        node.test.right.type === "Literal" &&
        node.test.right.value === 0
      ) {
        let containsPreviousValue = false;
        directC14Walk(node.consequent, (candidate) => {
          if (candidate === previousValues) containsPreviousValue = true;
        });
        if (containsPreviousValue) dominatingSlotGuards.push(node);
      }
    });
    if (dominatingSlotGuards.length !== 1) return reject();
    ...
    directC14AtTopology = Object.freeze({
      ...
      previousDigests,
      currentDigests,
      importedStates: new Set(importedStates.map(({ node }) => node)),
    });
    return directC14AtTopology;
  };
  const directC14JoinAtDomain = (receiver, indexes, memberName) => {
    if (
      receiver.arrayElements === null ||
      indexes.some(
        (index) =>
          ...
          index < 0 ||
          index >= receiver.arrayElements.length,
      )
    ) {
      fail(`unanalyzable string-bearing member transform ${memberName}`);
    }
    ...
    const element = joinAbstractValues(...elements);
    assertStaticStrings(element.staticStrings, "ordinary");
    return exposeSummaryValue(element);
  };
  const directC14LaunchFileValue = () => {
    const stringValue = makeCallerDerivedTypedValue("string", {
      unknownMemberValue: false,
    });
    const numberValue = makeCallerDerivedTypedValue("number", {
      unknownMemberValue: false,
    });
    const identityValue = makeValue("frozen", [], [], [], null, null, {
      callerDerived: true,
      typeofStrings: ["object"],
    });
    return makeValue(
  ...
  const directC14PinnedRecoveryStatesValue = () => {
    const state = makeValue("frozen", [], [], [], null, null, {
      known: true,
      typeofStrings: ["object"],
    });
    const states = Array.from({ length: 28 }, () => state);
    return makeValue("frozen", [], [], [], states, null, {
      known: true,
      arraySequence: true,
      sequenceShape: true,
      sequenceElement: state,
      indexableShape: true,
      indexElement: state,
      iterableShape: true,
      iterationElement: state,
      typeofStrings: ["object"],
    });
  };
  const directC14ExactAtSelection = (
    callNode,
    member,
    argumentValues,
    scope,
    ...
    const topology = directC14AtTopologyExact();
    if (topology === null) return null;
    if (callNode === topology.descriptors) {
      const domain = exactC14DescriptorsAtDomain(
        callNode,
        member,
        argumentValues,
        scope,
      ...
      return domain === null
        ? null
        : directC14JoinAtDomain(member.receiver, domain, "at");
    }
    if (callNode === topology.rights) {
      return directC14JoinAtDomain(
        member.receiver,
        Array.from({ length: 14 }, (_, index) => index),
        "at",
      );
    }
    if (callNode === topology.capsuleFile) {
      return directC14LaunchFileValue();
    }
    if (callNode === topology.values) {
      return directC14JoinAtDomain(
        member.receiver,
    ...
    if (callNode === topology.previousValues) {
      return directC14JoinAtDomain(
        member.receiver,
    ...
    if (
      callNode === topology.previousDigests ||
      callNode === topology.currentDigests
    ) {
      return directC14JoinAtDomain(
        member.receiver,
  ...
  const evaluateSafeMemberTransform = (
    member,
    argumentValues,
    context,
    callNode,
    ...
    if (memberName === "at") {
      ...
      const exactC14Selection = directC14ExactAtSelection(
        callNode,
        member,
        argumentValues,
        scope,
      ...
      if (exactC14Selection !== null) return exactC14Selection;
    ...
    if (memberName === "slice") {
      ...
      if (
        ...
        return makeValue("mutable-local", [], [], [], null, null, {
          ...
          iterableShape: receiver.compileTime.iterableShape,
  ...
  const evaluateCall = (node, scope, context) => {
    ...
    if (member.kind === "private-method") {
      ...
      const exactLocalArguments =
        node.arguments.length === 2 &&
        node.arguments.every(
          (argument) =>
            argument.type === "Identifier" &&
            resolve(scope, argument.name).kind === "local",
        );
      const legacyCommitNames =
        exactLocalArguments &&
  ...
  const evaluateNew = (node, scope, context) => {
    ...
    return makeValue(
      ...
      objectProperties,
      {
        ...
        stringConversions,
        iterableElements,
        iterableShape: binding.name === "Set",
        iterationElement:
          binding.name !== "Set"
            ? null
            : iterableElements === null
              ? makeCallerDerivedUnknownValue(callerDerived)
              : joinIterationElements(iterableElements),
        typeofStrings: ["object"],
        plusStrings: stringConversions,
      },
    );
  ...
  evaluateExpression = (node, scope, context = {}) => {
    ...
    if (node.type === "ArrayExpression") {
      ...
      return makeValue(
        ...
        null,
        {
          ...
          iterableShape: true,
          iterationElement: joinIterationElements(values),
          stringConversions,
          typeofStrings: ["object"],
          plusStrings: stringConversions,
        },
    ...
    if (node.type === "BinaryExpression") {
      ...
      const exactC14NumericAddition = directC14ExactNumericAddition(
        node,
        scope,
        expressionContext,
      );
      const effectiveStringCoercion =
        stringCoercion && !exactC14NumericAddition;
      ...
      const numericAddition =
        node.operator === "+" &&
        (!effectiveStringCoercion || exactC14NumericAddition) &&
        (exactC14NumericAddition ||
  ...
  const visitVariable = (
    ...
    if (forOf) {
      ...
      binding.forOfSource = forOfSource;
    ...
    if (module && requirementsDependencies.has(declarator.id.name)) {
      value = sanitizeRequirementsValue(value, "requirements-root");
    }
    if (binding.kind === "private-store") {
      if (
  ...
  const visitFunction = (node) => {
    ...
    if (
      directC14AuditedSource &&
      directC14ContextuallyReachedLocalFunctions.has(node.id.name) &&
      functionRecords.get(node.id.name)?.exported === false
    ) {
      return;
    }
    visitBlock(
...
function evaluateDirectC14Provenance(
  program,
  ...
  const candidateActivation =
    c14ProvenanceMode === DIRECT_C14_CANDIDATE_ACTIVATION_MODE;
  ...
  if (candidateActivation && !exactRootAvailable) {
    fail("REQUIREMENTS_ROOT_NOT_EXACT");
  }
  if (
...
function assertDirectC14FragmentProvenance(program) {
  return evaluateDirectC14Provenance(program, "fragment-control");
}

function assertDirectC14CandidateActivationProvenance(program) {
  return evaluateDirectC14Provenance(
    program,
...
function independentStaticAudit(
  ...
  if (c14ProvenanceMode === DIRECT_C14_CANDIDATE_ACTIVATION_MODE) {
    let fragmentRejection = null;
    try {
      assertDirectC14FragmentProvenance(program);
    } catch (error) {
  ...
  const scope = directAssertContextualGrammar(
    program,
    walkAudit.nodeCount,
    byteDigest(sourceBytes),
  );
  return Object.freeze({
    ...surface,
    ...scope,
...
function directProvenanceRequirementsSkeleton(
  ...
  let source = validSkeleton(extra, new Map([[functionName, functionBody]]));
...
function directCommittedProtectedProjectionBody(schemaPath, storeName) {
  return `const projection = deepFreeze(nullRecord([["schema", ${schemaPath}], ["authority", guardianContract.authority], ["physicalFacts", guardianContract.physicalFacts]])); const privateMetadata = deepFreeze(nullRecord([])); ${storeName}.set(projection, privateMetadata); return projection;`;
}

function directStateHashProjectionBody({ preimageMutation = null } = {}) {
  const exactEntries = [
  ...
  const preimageEntries = [...exactEntries];
  if (preimageMutation === "digest-too-early") {
    preimageEntries.push(
      '["stateSha256", guardianContract.startupMaps.recoveryOnlySha256]',
    );
  ...
  return `const selectedMode = guardianContract.modes.at(0); const selectedPhase = guardianContract.vocabularies.phases.at(0); const statePreimage = deepFreeze(nullRecord([${preimageEntries.join(", ")}])); const projectionDigest = sha256(canonicalJsonBytes(statePreimage)); const projection = deepFreeze(nullRecord([${exactEntries.join(", ")}, ["stateSha256", projectionDigest]])); const privateMetadata = deepFreeze(nullRecord([])); stateMetadata.set(projection, privateMetadata); return projection;`;
}

function directFrozenSourceShapePolicyControls() {
  const rejectShape =
    'function rejectDirectShape() { throw new Error("CONTROL_SHAPE"); }';
  const rejectBounds =
    'function rejectDirectBounds() { throw new Error("CONTROL_BOUNDS"); }';
  const rejectBrand =
    'function rejectDirectBrand() { throw new Error("CONTROL_BINDING"); } function rejectDirectRecord() { throw new Error("CONTROL_SHAPE"); }';
  const decodeStatus =
    'const decodedFrame = decodeCanonicalJsonLine(statusFrameBytes, "status frame", guardianContract.limits.statusFrameMaximumBytes, rejectDirectShape);';
  const normalizeSelection = `${decodeStatus} const recoverySelection = exactRecord(decodedFrame.value, guardianContract.frameFields.recoverySelection, "recovery selection", rejectDirectShape);`;
  const verifyLaunch = `${decodeStatus} const admission = exactRecord(decodedFrame.value, guardianContract.frameFields.ADMIT, "admission frame", rejectDirectShape); const launchBytes = decodeCanonicalBase64(admission.launchCapsuleV3, "launch capsule", guardianContract.limits.admissionFrameMaximumBytes, rejectDirectShape); const launchResult = verifyCandidateContainmentLaunchCapsuleV3(launchBytes);`;
  return Object.freeze([
    Object.freeze({
      id: "direct-requirements-field-list-normalizer-leaf",
      accepted: true,
      source: directProvenanceRequirementsSkeleton(
        `${decodeStatus} const status = exactRecord(decodedFrame.value, guardianContract.frameFields.STATUS, "status frame", rejectDirectShape); return status.schema === guardianContract.schemas.wireFrame;`,
        rejectShape,
      ),
    }),
    Object.freeze({
      id: "direct-requirements-field-list-whole-record",
      accepted: false,
      expectedRejection:
        "direct static gate: contextual provenance REQUIREMENTS_PATH_ESCAPE",
      source: directProvenanceRequirementsSkeleton(
        `${decodeStatus} const status = exactRecord(decodedFrame.value, guardianContract.frameFields, "status frame", rejectDirectShape); return status.schema;`,
        rejectShape,
      ),
    }),
    Object.freeze({
      id: "direct-requirements-numeric-bound-leaf",
      accepted: true,
      source: directProvenanceRequirementsSkeleton(
        'return boundedInteger(1, "status count", 0, guardianContract.limits.maximumStatusFramesPerTransition, rejectDirectBounds);',
        rejectBounds,
      ),
    }),
    Object.freeze({
      id: "direct-requirements-numeric-bound-whole-record",
      accepted: false,
      expectedRejection:
        "direct static gate: contextual provenance REQUIREMENTS_PATH_ESCAPE",
      source: directProvenanceRequirementsSkeleton(
        'return boundedInteger(1, "status count", 0, guardianContract.limits, rejectDirectBounds);',
        rejectBounds,
      ),
    }),
    Object.freeze({
      id: "direct-requirements-strict-equality-leaves",
      accepted: true,
      source: directProvenanceRequirementsSkeleton(
        "const schemaPair = guardianContract.schemas.inputProjection !== guardianContract.schemas.stateProjection; const mapPair = guardianContract.startupMaps.normalSha256 !== guardianContract.startupMaps.recoveryOnlySha256; const vocabularyPair = guardianContract.vocabularies.controlLevel !== guardianContract.vocabularies.controlType; return schemaPair && mapPair && vocabularyPair;",
      ),
    }),
    Object.freeze({
      id: "direct-requirements-strict-equality-whole-subtree",
      accepted: false,
      expectedRejection:
        "direct static gate: contextual provenance REQUIREMENTS_PATH_ESCAPE",
      source: directProvenanceRequirementsSkeleton(
        "return guardianContract.startupMaps === guardianContract.startupMaps;",
      ),
    }),
    Object.freeze({
      id: "direct-requirements-array-receiver-leaves",
      accepted: true,
      source: directProvenanceRequirementsSkeleton(
        'const modeKnown = guardianContract.modes.includes("RECOVERY_ONLY"); const inputKnown = guardianContract.vocabularies.inputKinds.includes("CANCEL"); const firstInputField = guardianContract.frameFields.inputProjection.at(0); return modeKnown && inputKnown && firstInputField === "schema";',
      ),
    }),
    Object.freeze({
      id: "direct-requirements-array-receiver-whole-record",
      accepted: false,
      expectedRejection:
        "direct static gate: contextual provenance REQUIREMENTS_PATH_ESCAPE",
      source: directProvenanceRequirementsSkeleton(
        'return guardianContract.frameFields.includes("schema");',
      ),
    }),
    Object.freeze({
      id: "direct-requirements-ternary-selected-numeric-leaves",
      accepted: true,
      source: directProvenanceRequirementsSkeleton(
        'const recovery = guardianContract.modes.includes("RECOVERY_ONLY"); const descriptorCount = recovery ? guardianContract.startupMaps.recoveryOnlyDescriptorCount : guardianContract.startupMaps.normalDescriptorCount; return boundedInteger(descriptorCount, "descriptor count", 0, guardianContract.limits.admissionRightsCount, rejectDirectBounds);',
        rejectBounds,
      ),
    }),
    Object.freeze({
      id: "direct-requirements-ternary-selected-whole-record",
      accepted: false,
      expectedRejection:
        "direct static gate: contextual provenance REQUIREMENTS_PATH_ESCAPE",
      source: directProvenanceRequirementsSkeleton(
        'const recovery = guardianContract.modes.includes("RECOVERY_ONLY"); const descriptorCount = recovery ? guardianContract.startupMaps.recoveryOnlyDescriptorCount : guardianContract.startupMaps; return boundedInteger(descriptorCount, "descriptor count", 0, guardianContract.limits.admissionRightsCount, rejectDirectBounds);',
        rejectBounds,
      ),
    }),
    Object.freeze({
      id: "direct-protected-output-startup-authority-physical-facts",
      accepted: true,
      source: directProvenanceRequirementsSkeleton(
        directCommittedProtectedProjectionBody(
          "guardianContract.schemas.startupProjection",
          "startupMetadata",
        ),
        "",
        "createCandidateContainmentGuardianStartupV1",
    ...
    Object.freeze({
      id: "direct-protected-output-input-authority-physical-facts",
      accepted: true,
      source: directProvenanceRequirementsSkeleton(
        directCommittedProtectedProjectionBody(
          "guardianContract.schemas.inputProjection",
          "inputMetadata",
        ),
        "",
        "createCandidateContainmentGuardianControllerClosedInputV1",
    ...
    Object.freeze({
      id: "direct-protected-output-state-authority-physical-facts",
      accepted: true,
      source: directProvenanceRequirementsSkeleton(
        directCommittedProtectedProjectionBody(
          "guardianContract.schemas.stateProjection",
          "stateMetadata",
        ),
        "",
        "initializeCandidateContainmentGuardianControlV1",
    ...
    Object.freeze({
      id: "direct-protected-output-status-authority-physical-facts",
      accepted: true,
      source: directProvenanceRequirementsSkeleton(
        'const safeBytes = copyBoundedBuffer(statusFrameBytes, "status frame", { minimumBytes: 0, maximumBytes: guardianContract.limits.statusFrameMaximumBytes }, rejectDirectBounds); return frozenCopyOnReadBytes(safeBytes, [["schema", guardianContract.schemas.statusArtifact], ["authority", guardianContract.authority], ["physicalFacts", guardianContract.physicalFacts]]);',
        rejectBounds,
      ),
    }),
    Object.freeze({
      id: "direct-protected-output-swapped-authority-physical-facts",
      accepted: false,
      expectedRejection:
        "direct static gate: contextual provenance AUTHORITY_WRONG_PLACEMENT",
      source: directProvenanceRequirementsSkeleton(
        'return deepFreeze(nullRecord([["schema", guardianContract.schemas.statusArtifact], ["authority", guardianContract.physicalFacts], ["physicalFacts", guardianContract.authority]]));',
      ),
    }),
    Object.freeze({
      id: "direct-state-exact-unsigned-preimage-hash",
      accepted: true,
      source: directProvenanceRequirementsSkeleton(
        directStateHashProjectionBody(),
        "",
        "initializeCandidateContainmentGuardianControlV1",
      ),
    }),
    Object.freeze({
      id: "direct-state-inexact-unsigned-preimage-hash",
      accepted: false,
      expectedRejection:
        "direct static gate: contextual provenance STATE_PREIMAGE_NOT_EXACT",
      source: directProvenanceRequirementsSkeleton(
        directStateHashProjectionBody({
          preimageMutation: "digest-too-early",
        }),
        "",
        "initializeCandidateContainmentGuardianControlV1",
      ),
    ...
    Object.freeze({
      id: "direct-state-omitted-field-preimage-hash",
      accepted: false,
      expectedRejection:
        "direct static gate: contextual provenance STATE_PREIMAGE_NOT_EXACT",
      source: directProvenanceRequirementsSkeleton(
        directStateHashProjectionBody({ preimageMutation: "omitted" }),
        "",
        "initializeCandidateContainmentGuardianControlV1",
      ),
    }),
    Object.freeze({
      id: "direct-state-reordered-field-preimage-hash",
      accepted: false,
      expectedRejection:
        "direct static gate: contextual provenance STATE_PREIMAGE_NOT_EXACT",
      source: directProvenanceRequirementsSkeleton(
        directStateHashProjectionBody({ preimageMutation: "reordered" }),
        "",
        "initializeCandidateContainmentGuardianControlV1",
      ),
    }),
    Object.freeze({
      id: "direct-state-substituted-schema-preimage-hash",
      accepted: false,
      expectedRejection:
        "direct static gate: contextual provenance STATE_PREIMAGE_NOT_EXACT",
      source: directProvenanceRequirementsSkeleton(
        directStateHashProjectionBody({ preimageMutation: "substituted" }),
        "",
        "initializeCandidateContainmentGuardianControlV1",
      ),
    }),
    Object.freeze({
      id: "direct-state-arbitrary-short-preimage-hash",
      accepted: false,
      expectedRejection:
        "direct static gate: contextual provenance STATE_PREIMAGE_NOT_EXACT",
      source: directProvenanceRequirementsSkeleton(
        directStateHashProjectionBody({ preimageMutation: "arbitrary" }),
        "",
        "initializeCandidateContainmentGuardianControlV1",
      ),
    }),
    Object.freeze({
      id: "direct-pinned-import-normalized-dynamic-membership",
      accepted: true,
      source: directProvenanceRequirementsSkeleton(
        `${normalizeSelection} const planKnown = CANDIDATE_CONTAINMENT_RECOVERY_PLAN_STATUSES_V1.includes(recoverySelection.planStatus); const actorKnown = CANDIDATE_CONTAINMENT_RECOVERY_ACTOR_KINDS_V1.includes(recoverySelection.actorKind); const dispositionKnown = CANDIDATE_CONTAINMENT_RECOVERY_DISPOSITIONS_V1.includes(recoverySelection.disposition); const locationKnown = CANDIDATE_CONTAINMENT_RECOVERY_SOURCE_LOCATIONS_V1.includes(recoverySelection.sourceLocation); const quarantineKnown = CANDIDATE_CONTAINMENT_RECOVERY_QUARANTINE_REASONS_V1.includes(recoverySelection.quarantineReason); return planKnown && actorKnown && dispositionKnown && locationKnown && quarantineKnown;`,
        rejectShape,
      ),
    }),
    Object.freeze({
      id: "direct-pinned-import-raw-dynamic-membership",
      accepted: false,
      expectedRejection:
        "direct static gate: contextual provenance PINNED_IMPORT_ESCAPE",
      source: directProvenanceRequirementsSkeleton(
        `${normalizeSelection} return CANDIDATE_CONTAINMENT_RECOVERY_PLAN_STATUSES_V1.includes(decodedFrame.value.planStatus);`,
        rejectShape,
      ),
    }),
    Object.freeze({
      id: "direct-pinned-recovery-digest-normalized-comparison",
      accepted: true,
      source: directProvenanceRequirementsSkeleton(
        `${normalizeSelection} return recoverySelection.recoveryRequirementsSha256 === CANDIDATE_CONTAINMENT_RECOVERY_REQUIREMENTS_SHA256_V1;`,
        rejectShape,
      ),
    }),
    Object.freeze({
      id: "direct-pinned-recovery-digest-direct-escape",
      accepted: false,
      expectedRejection:
        "direct static gate: contextual provenance PINNED_IMPORT_ESCAPE",
      source: validSkeleton(
        ...
        new Map([
          [
            "verifyCandidateContainmentGuardianStatusFrameV1",
            "return CANDIDATE_CONTAINMENT_RECOVERY_REQUIREMENTS_SHA256_V1;",
          ],
        ]),
      ),
    }),
    Object.freeze({
      id: "direct-pinned-launch-verifier-projection-chain",
      accepted: true,
      source: directProvenanceRequirementsSkeleton(
        `${verifyLaunch} const selectedFile = directSelectLaunchFile(launchResult); const fileIdentity = directNormalizeLaunchIdentity(selectedFile); const fileIdentitySha256 = directDigestLaunchIdentity(fileIdentity); const requirementsEqual = launchResult.requirementsSha256 === CANDIDATE_CONTAINMENT_LAUNCH_REQUIREMENTS_SHA256_V3; const rawEqual = launchResult.rawSha256 === admission.launchCapsuleV3Sha256; const identityEqual = fileIdentity.role === selectedFile.role && fileIdentity.byteLength === selectedFile.byteLength && fileIdentity.sha256 === selectedFile.sha256; return requirementsEqual && rawEqual && identityEqual && fileIdentitySha256.length === 64;`,
        `${rejectShape} function directSelectLaunchFile(value) { return value.files.at(0); } function directNormalizeLaunchIdentity(value) { return deepFreeze(nullRecord([["role", value.role], ["byteLength", value.byteLength], ["sha256", value.sha256]])); } function directDigestLaunchIdentity(value) { return sha256(canonicalJsonBytes(value)); }`,
      ),
    }),
    Object.freeze({
      id: "direct-pinned-launch-verifier-bounded-dynamic-file-projection",
      accepted: true,
      source: directProvenanceRequirementsSkeleton(
        `${verifyLaunch} const right = exactRecord(decodedFrame.value, guardianContract.frameFields.admissionRight, "admission right", rejectDirectShape); const index = boundedInteger(right.index, "admission right index", 0, guardianContract.limits.admissionRightsCount, rejectDirectBounds); const selectedFile = launchResult.files.at(index); const identitySha256 = sha256(canonicalJsonBytes(selectedFile.identity)); const requirementsEqual = launchResult.requirementsSha256 === CANDIDATE_CONTAINMENT_LAUNCH_REQUIREMENTS_SHA256_V3; const fileEqual = right.role === selectedFile.role && right.byteLength === selectedFile.byteLength && right.contentSha256 === selectedFile.sha256 && right.launchFileIdentitySha256 === identitySha256 && right.currentOffset === selectedFile.initialOffset; return requirementsEqual && fileEqual;`,
        `${rejectShape} ${rejectBounds}`,
      ),
    }),
    Object.freeze({
      id: "direct-pinned-launch-verifier-unbounded-dynamic-file-index",
      accepted: false,
      expectedRejection:
        "direct static gate: contextual provenance PINNED_IMPORT_ESCAPE",
      source: directProvenanceRequirementsSkeleton(
        `${verifyLaunch} const selectedFile = launchResult.files.at(admission.sequence); return selectedFile.role === admission.action;`,
        rejectShape,
      ),
    }),
    Object.freeze({
      id: "direct-pinned-launch-verifier-identity-direct-escape",
      accepted: false,
      expectedRejection:
        "direct static gate: contextual provenance PINNED_IMPORT_ESCAPE",
      source: directProvenanceRequirementsSkeleton(
        `${verifyLaunch} const index = boundedInteger(admission.sequence, "launch file index", 0, guardianContract.limits.admissionRightsCount, rejectDirectBounds); const selectedFile = launchResult.files.at(index); return selectedFile.identity;`,
        `${rejectShape} ${rejectBounds}`,
      ),
    }),
    Object.freeze({
      id: "direct-pinned-launch-verifier-identity-wrong-member",
      accepted: false,
      expectedRejection:
        "direct static gate: contextual provenance PINNED_IMPORT_MEMBER_NOT_ALLOWED",
      source: directProvenanceRequirementsSkeleton(
        `${verifyLaunch} const index = boundedInteger(admission.sequence, "launch file index", 0, guardianContract.limits.admissionRightsCount, rejectDirectBounds); const selectedFile = launchResult.files.at(index); return selectedFile.identity.environment;`,
        `${rejectShape} ${rejectBounds}`,
      ),
    }),
    Object.freeze({
      id: "direct-pinned-launch-digest-direct-escape",
      accepted: false,
      expectedRejection:
        "direct static gate: contextual provenance PINNED_IMPORT_ESCAPE",
      source: validSkeleton(
        ...
        new Map([
          [
            "verifyCandidateContainmentGuardianStatusFrameV1",
            "return CANDIDATE_CONTAINMENT_LAUNCH_REQUIREMENTS_SHA256_V3;",
          ],
        ]),
      ),
    }),
    Object.freeze({
      id: "direct-pinned-launch-verifier-result-direct-escape",
      accepted: false,
      expectedRejection:
        "direct static gate: contextual provenance PINNED_IMPORT_ESCAPE",
      source: directProvenanceRequirementsSkeleton(
        `${verifyLaunch} return launchResult;`,
        rejectShape,
      ),
    }),
    Object.freeze({
      id: "direct-pinned-launch-verifier-wrong-member",
      accepted: false,
      expectedRejection:
        "direct static gate: contextual provenance PINNED_IMPORT_MEMBER_NOT_ALLOWED",
      source: directProvenanceRequirementsSkeleton(
        `${verifyLaunch} return launchResult.environment;`,
        rejectShape,
      ),
    }),
    Object.freeze({
      id: "direct-pinned-launch-verifier-file-direct-escape",
      accepted: false,
      expectedRejection:
        "direct static gate: contextual provenance PINNED_IMPORT_ESCAPE",
      source: directProvenanceRequirementsSkeleton(
        `${verifyLaunch} const selectedFile = launchResult.files.at(0); return selectedFile;`,
        rejectShape,
      ),
    }),
    Object.freeze({
      id: "direct-local-helper-normalize-first-nested-value",
      accepted: true,
      source: directProvenanceRequirementsSkeleton(
        `${normalizeSelection} return directReadPlanStatus(recoverySelection);`,
        `${rejectShape} function directReadPlanStatus(value) { return value.planStatus; }`,
      ),
    }),
    Object.freeze({
      id: "direct-local-helper-raw-nested-value",
      accepted: false,
      expectedRejection:
        "direct static gate: contextual provenance LOCAL_CALLER_UNTRUSTED",
      source: directProvenanceRequirementsSkeleton(
        `${normalizeSelection} return directReadPlanStatus(decodedFrame.value);`,
        `${rejectShape} function directReadPlanStatus(value) { return value.planStatus; }`,
      ),
    }),
    Object.freeze({
      id: "direct-local-helper-branded-exact-record",
      accepted: true,
      source: directProvenanceRequirementsSkeleton(
        'const brandPresent = startupMetadata.has(startupProjection); exactBoolean(brandPresent, true, "startup projection brand", rejectDirectBrand); const privateMetadata = startupMetadata.get(startupProjection); const startup = exactRecord(startupProjection, guardianContract.frameFields.startupProjection, "startup projection", rejectDirectRecord); return directReadBrandedSchema(startup);',
        `${rejectBrand} function directReadBrandedSchema(value) { return value.schema; }`,
      ),
    }),
    Object.freeze({
      id: "direct-local-helper-copied-and-decoded-bytes",
      accepted: true,
      source: directProvenanceRequirementsSkeleton(
        'const safeCopy = copyBoundedBuffer(statusFrameBytes, "status frame", { minimumBytes: 0, maximumBytes: guardianContract.limits.statusFrameMaximumBytes }, rejectDirectShape); const canonicalFrame = decodeCanonicalJsonLine(statusFrameBytes, "status frame", guardianContract.limits.statusFrameMaximumBytes, rejectDirectShape); return directTrustedByteLength(safeCopy) + directTrustedByteLength(canonicalFrame.bytes);',
        `${rejectShape} function directTrustedByteLength(value) { return value.length; }`,
      ),
    }),
    Object.freeze({
      id: "direct-local-helper-protected-import-result",
      accepted: true,
      source: validSkeleton(
        "function directImportedStateLength(value) { return value.length; }",
        new Map([
          [
            "verifyCandidateContainmentGuardianStatusFrameV1",
            "const importedState = CANDIDATE_CONTAINMENT_RECOVERY_RECORD_STATES_V1.at(0); return directImportedStateLength(importedState);",
          ],
        ]),
      ),
    }),
    Object.freeze({
      id: "direct-local-helper-generated-normalized-transitive-callers",
      accepted: true,
      source: directProvenanceRequirementsSkeleton(
        `${decodeStatus} const normalizedStatus = exactRecord(decodedFrame.value, guardianContract.frameFields.STATUS, "status frame", rejectDirectShape); const generatedStatus = deepFreeze(nullRecord([["schema", guardianContract.schemas.wireFrame]])); const normalizedLength = directFromNormalized(normalizedStatus); const generatedLength = directFromGenerated(generatedStatus); return normalizedLength + generatedLength;`,
        `${rejectShape} function directSchemaLeaf(value) { return value.schema; } function directSchemaLength(value) { return directSchemaLeaf(value).length; } function directFromNormalized(value) { return directSchemaLength(value); } function directFromGenerated(value) { return directSchemaLength(value); }`,
      ),
    }),
    Object.freeze({
      id: "direct-local-helper-transitive-untrusted-caller",
      accepted: false,
      expectedRejection:
        "direct static gate: contextual provenance LOCAL_CALLER_UNTRUSTED",
      source: directProvenanceRequirementsSkeleton(
        `${decodeStatus} return directUntrustedWrapper(decodedFrame.value);`,
        `${rejectShape} function directUntrustedLeaf(value) { return value.schema; } function directUntrustedWrapper(value) { return directUntrustedLeaf(value); }`,
      ),
    }),
    Object.freeze({
      id: "direct-local-helper-transitive-mixed-callers",
      accepted: false,
      expectedRejection:
        "direct static gate: contextual provenance LOCAL_CALLER_MIXED",
      source: directProvenanceRequirementsSkeleton(
        `${decodeStatus} const normalizedStatus = exactRecord(decodedFrame.value, guardianContract.frameFields.STATUS, "status frame", rejectDirectShape); const trusted = directMixedTrustedWrapper(normalizedStatus); const raw = directMixedRawWrapper(decodedFrame.value); return trusted + raw;`,
        `${rejectShape} function directMixedLeaf(value) { return value.schema; } function directMixedTrustedWrapper(value) { return directMixedLeaf(value); } function directMixedRawWrapper(value) { return directMixedLeaf(value); }`,
      ),
    }),
    Object.freeze({
      id: "direct-local-failure-callback-normalizer-argument",
      accepted: true,
      source: directProvenanceRequirementsSkeleton(
        'const canonicalFrame = decodeCanonicalJsonLine(statusFrameBytes, "status frame", guardianContract.limits.statusFrameMaximumBytes, directExactFailure); const status = exactRecord(canonicalFrame.value, guardianContract.frameFields.STATUS, "status frame", directExactFailure); return status.schema;',
        'function directExactFailure() { throw new Error("CONTROL_SHAPE"); }',
      ),
    }),
    Object.freeze({
      id: "direct-local-failure-callback-alias",
      accepted: false,
      expectedRejection:
        "direct static gate: contextual provenance LOCAL_CALL_INDIRECT",
      source: directProvenanceRequirementsSkeleton(
        'const callback = directExactFailure; const canonicalFrame = decodeCanonicalJsonLine(statusFrameBytes, "status frame", guardianContract.limits.statusFrameMaximumBytes, callback); return canonicalFrame.bytes.length;',
        'function directExactFailure() { throw new Error("CONTROL_SHAPE"); }',
      ),
    }),
  ]);
...
function directProvenancePolicyControls() {
  ...
  return Object.freeze([
    ...
    ...directFrozenSourceShapePolicyControls(),
  ]);
}
...
function observeDirectProvenancePolicyControls() {
  ...
  return controls.map(({ id, accepted, expectedRejection = null, source }) => {
    ...
    try {
      directParse(source, DIRECT_ACORN_PARSE_OPTIONS);
    } catch {
      sourceParsed = false;
    }
    let rejection = null;
...
const EXPECTED_DIRECT_C14_CANDIDATE_ACTIVATION_PROJECTION_SHA256 =
  "38fe7b40ab848afd0abf06ed886c39f6467ce5169ce5c6253db2953106f64fde";
...
function directCandidateActivationStateBody({
  ...
  if (wrongTransitionState) {
    ...
    body = body.replace(
      "; const metadata = stateProjectionMetadata(projection);",
      `; const wrongProjection = ${projectionExpression}; const transition = deepFreeze(nullRecord([["state", wrongProjection]])); const metadata = stateProjectionMetadata(projection);`,
    );
    assert.equal(body.includes("return projection;"), true);
    body = body.replace("return projection;", "return transition;");
  }
  return body;
}
...
function directCandidateActivationControls() {
  ...
  return Object.freeze([
    ...
    Object.freeze({
      id: "direct-candidate-returned-transition-has-distinct-state",
      source: directCandidateActivationSkeleton({
    ...
    Object.freeze({
      id: "direct-candidate-multivalued-slice-index-rejected",
      source: directCandidateActivationSkeleton({
        verificationBody:
          "const index = startupProjection ? 0 : 1; const values = [0, 1]; const sliced = values.slice(index); return sliced;",
      }),
      expectedLegacyAccepted: false,
      expectedLegacyRejection:
        "direct static gate: contextual unanalyzable string-bearing member transform slice",
      expectedCandidateAccepted: false,
      expectedCandidateRejection: null,
    }),
    Object.freeze({
      id: "direct-candidate-multivalued-at-mutable-join-rejected",
      source: directCandidateActivationSkeleton({
        verificationBody:
          "const index = guardianContract.limits.startupReportMaximumBytes ? 0 : 1; const values = [[], null]; return values.at(index);",
      }),
      expectedLegacyAccepted: false,
      expectedLegacyRejection:
        "direct static gate: contextual unanalyzable string-bearing member transform at",
      expectedCandidateAccepted: false,
      expectedCandidateRejection: null,
    }),
    Object.freeze({
...
function assertDirectCandidateActivationContract() {
  ...
  for (const { source, expectedLegacyRejection } of controls) {
    if (expectedLegacyRejection !== undefined) {
      assert.throws(() => independentStaticAudit(Buffer.from(source, "utf8")), {
...
test("defines the independent provenance policy before implementation", () => {
  ...
  assert.deepEqual(
    observations.map(
      ({ id, actualAccepted, rejectionStageMatched, sourceParsed }) => ({
        id,
...
function contextualPositive() {
  ...
  const returnAndContainerPrecisionPositiveSources = [
    ...
    approvedOneParameterExportSource(
      "const selected = currentState ? Number(currentState) : Boolean(currentState); const next = selected + 1 + 1; return next;",
    ),
    approvedOneParameterExportSource(
      "const left = new Set([Number(currentState)]); const right = new Set([1, 2]); const selected = currentState ? left : right; for (const value of selected) { const next = value + 1 + 1; } return null;",
    ),
    approvedOneParameterExportSource(
      "const left = new Set([Number(currentState)]); const right = [1]; const selected = currentState ? left : right; for (const value of selected) { const next = value + 1 + 1; } return null;",
    ),
    approvedOneParameterExportSource(
      "const left = new Set([Number(currentState)]); const right = canonicalJsonBytes(null); const selected = currentState ? left : right; for (const value of selected) { const next = value + 1 + 1; } return null;",
    ),
    approvedOneParameterExportSourceWithExtra(
      "for (const value of localSelectedSet(currentState)) { const next = value + 1 + 1; } return null;",
      "function localSelectedSet(value) { if (value) { return new Set([Number(value)]); } return new Set([1, 2]); }",
    ),
    approvedOneParameterExportSource(
      'const left = new Set(["safe"]); const right = ["bounded"]; const selected = currentState ? left : right; for (const value of selected) { const kind = typeof value; const next = kind.length + 1 + 1; } return null;',
    ),
    approvedOneParameterExportSource(
      "const record = frozenCopyOnReadBytes(canonicalJsonBytes(null), currentState); const next = record.bytes.length + 1 + 1; return next;",
    ),
    approvedOneParameterExportSource(
      'const record = frozenCopyOnReadBytes(canonicalJsonBytes(null), [["schema", "safe"]]); const kind = typeof record.value; const next = kind.length + 1 + 1; return next;',
    ),
    approvedOneParameterExportSource(
      'const left = frozenCopyOnReadBytes(canonicalJsonBytes(null), [["value", Number(currentState)]]); const right = frozenCopyOnReadBytes(canonicalJsonBytes(null), [["value", 1]]); const selected = currentState ? left : right; const next = selected.value + 1 + 1; return next;',
    ),
    approvedOneParameterExportSource(
      "const left = [Number(currentState), 1].slice(0, 1); const right = canonicalJsonBytes(null); const selected = currentState ? left : right; for (const value of selected) { const next = value + 1 + 1; } return null;",
    ),
    approvedOneParameterExportSource(
  ...
  const iterableJoinProvenanceMutationSources = [
    [
      "direct-unequal-set-elements-plus-node",
      approvedOneParameterExportSource(
        'const left = new Set(["n"]); const right = new Set(["x", "y"]); const selected = currentState ? left : right; for (const value of selected) { const hidden = value + "ode:fs"; } return null;',
      ),
    ],
    [
      "direct-reversed-unequal-set-elements-plus-node",
      approvedOneParameterExportSource(
        'const left = new Set(["x", "y"]); const right = new Set(["n"]); const selected = currentState ? left : right; for (const value of selected) { const hidden = value + "ode:fs"; } return null;',
      ),
    ],
    [
      "direct-set-array-elements-plus-node",
      approvedOneParameterExportSource(
        'const left = new Set(["n"]); const right = ["x"]; const selected = currentState ? left : right; for (const value of selected) { const hidden = value + "ode:fs"; } return null;',
      ),
    ],
    [
      "direct-array-set-elements-plus-node",
      approvedOneParameterExportSource(
        'const left = ["x"]; const right = new Set(["n"]); const selected = currentState ? left : right; for (const value of selected) { const hidden = value + "ode:fs"; } return null;',
      ),
    ],
    [
      "direct-set-known-buffer-elements-plus-node",
      approvedOneParameterExportSource(
        'const left = new Set(["n"]); const right = canonicalJsonBytes(null); const selected = currentState ? left : right; for (const value of selected) { const hidden = value + "ode:fs"; } return null;',
      ),
    ],
    [
      "direct-known-buffer-set-elements-plus-node",
      approvedOneParameterExportSource(
        'const left = canonicalJsonBytes(null); const right = new Set(["n"]); const selected = currentState ? left : right; for (const value of selected) { const hidden = value + "ode:fs"; } return null;',
      ),
    ],
    [
      "direct-local-return-unequal-set-elements-plus-node",
      approvedOneParameterExportSourceWithExtra(
        'for (const value of localSelectedSet(currentState)) { const hidden = value + "ode:fs"; } return null;',
        'function localSelectedSet(value) { if (value) { return new Set(["n"]); } return new Set(["x", "y"]); }',
      ),
    ],
  ];
  assert.equal(iterableJoinProvenanceMutationSources.length, 7);
  for (const [id, sourceText] of iterableJoinProvenanceMutationSources) {
    kill(id, () => independentStaticAudit(asBytes(sourceText)));
  }
  const slicedIterableJoinMutationSources = [
    [
      "direct-sliced-array-buffer-elements-plus-node",
      approvedOneParameterExportSource(
        'const left = ["n", "x"].slice(0, 1); const right = canonicalJsonBytes(null); const selected = currentState ? left : right; for (const value of selected) { const hidden = value + "ode:fs"; } return null;',
      ),
    ],
    [
      "direct-buffer-sliced-array-elements-plus-node",
      approvedOneParameterExportSource(
        'const left = canonicalJsonBytes(null); const right = ["n", "x"].slice(0, 1); const selected = currentState ? left : right; for (const value of selected) { const hidden = value + "ode:fs"; } return null;',
      ),
    ],
    [
      "direct-unequal-sliced-array-elements-plus-node",
      approvedOneParameterExportSource(
        'const left = ["n", "x"].slice(0, 1); const right = ["x", "y"].slice(0, 2); const selected = currentState ? left : right; for (const value of selected) { const hidden = value + "ode:fs"; } return null;',
      ),
    ],
    [
      "direct-sliced-array-set-elements-plus-node",
      approvedOneParameterExportSource(
        'const left = ["n", "x"].slice(0, 1); const right = new Set(["x"]); const selected = currentState ? left : right; for (const value of selected) { const hidden = value + "ode:fs"; } return null;',
      ),
    ],
  ];
  assert.equal(slicedIterableJoinMutationSources.length, 4);
  ...
  const partialRecordMemberMutationSources = [
    [
      ...
      approvedOneParameterExportSource(
        'const record = frozenCopyOnReadBytes(canonicalJsonBytes(null), currentState); const hidden = String(record.value).at(0) + "ode:fs"; return null;',
      ),
    ],
    [
      "direct-dynamic-frozen-copy-member-plus-node",
      approvedOneParameterExportSource(
        'const record = frozenCopyOnReadBytes(canonicalJsonBytes(null), currentState); const hidden = record.value + "ode:fs"; return null;',
      ),
    ],
    [
      "direct-nested-dynamic-frozen-copy-member-plus-node",
      approvedOneParameterExportSource(
        'const record = frozenCopyOnReadBytes(canonicalJsonBytes(null), currentState); const nested = deepFreeze({ value: record }); const hidden = nested.value.value + "ode:fs"; return null;',
      ),
    ],
    [
      "direct-joined-dynamic-frozen-copy-member-plus-node",
      approvedOneParameterExportSource(
        'const left = frozenCopyOnReadBytes(canonicalJsonBytes(null), currentState); const right = frozenCopyOnReadBytes(canonicalJsonBytes(null), [["value", "safe"]]); const selected = currentState ? left : right; const hidden = selected.value + "ode:fs"; return null;',
      ),
    ],
  ];
  assert.equal(partialRecordMemberMutationSources.length, 4);
