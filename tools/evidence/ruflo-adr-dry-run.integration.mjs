...
test("official Ruflo parser sees the exact numeric ADR graph", () => {
  ...
  assert.equal(report.official.total, 39);
  assert.equal(report.staging.files.length, 39);
  ...
  assert.equal(report.official.edges, 221);
  ...
  assert.equal(report.integrity.uniqueIds, 39);
  assert.deepEqual(report.integrity.duplicateIds, []);
  assert.deepEqual(report.integrity.unknownStatuses, []);
  assert.equal(report.integrity.rawEdges, 221);
  assert.equal(report.integrity.uniqueEdges, 221);
