...
test("official Ruflo parser sees the exact numeric ADR graph", () => {
  ...
  assert.equal(report.official.total, 35);
  assert.equal(report.staging.files.length, 35);
  ...
  assert.equal(report.official.edges, 192);
  ...
  assert.equal(report.integrity.uniqueIds, 35);
  assert.deepEqual(report.integrity.duplicateIds, []);
  assert.deepEqual(report.integrity.unknownStatuses, []);
  assert.equal(report.integrity.rawEdges, 192);
  assert.equal(report.integrity.uniqueEdges, 192);
