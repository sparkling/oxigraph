...
test("official Ruflo parser sees the exact numeric ADR graph", () => {
  ...
  assert.equal(report.official.total, 34);
  assert.equal(report.staging.files.length, 34);
  ...
  assert.equal(report.official.edges, 188);
  ...
  assert.equal(report.integrity.uniqueIds, 34);
  assert.deepEqual(report.integrity.duplicateIds, []);
  assert.deepEqual(report.integrity.unknownStatuses, []);
  assert.equal(report.integrity.rawEdges, 188);
  assert.equal(report.integrity.uniqueEdges, 188);
