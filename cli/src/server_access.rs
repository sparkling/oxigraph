//! Server-layer listener validation (ADR-0026).
//! Proxy mode is selected only after successful access-policy validation.
...
impl ServerListener {
    ...
    pub fn resolve(bind: String, allow_remote: bool, proxy_profile: bool) -> Result<Self> {
        let addresses: Vec<_> = bind.to_socket_addrs()?.collect();
        validate_addresses(&addresses, allow_remote || proxy_profile)?;
        if !proxy_profile && addresses.iter().any(|address| !address.ip().is_loopback()) {
            eprintln!(
...
mod tests {
    ...
    fn resolution_retains_the_exact_validated_sockets() -> Result<()> {
        let listener = ServerListener::resolve("127.0.0.1:0".into(), false, false)?;
        ...
        let remote = ServerListener::resolve("192.0.2.1:7878".into(), true, false)?;
        assert_eq!(remote.addresses(), &["192.0.2.1:7878".parse()?]);
        assert!(ServerListener::resolve("invalid bind address".into(), true, false).is_err());
        Ok(())
    }
}
