...
/// then distinct-buffer rows, group-buffer rows, then aggregate-distinct keys.
...
fn ensure_update_budget(evaluator: &QueryEvaluator) -> Result<(), UpdateEvaluationError> {
    ...
    if let Some(budget) = evaluator.aggregate_distinct_budget() {
        budget.check()?;
    }
    Ok(())
}
...
fn can_silence_load_error(error: &UpdateEvaluationError) -> bool {
    if matches!(
        error,
        UpdateEvaluationError::TimedOut
            | UpdateEvaluationError::Cancelled
